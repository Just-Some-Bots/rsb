...
from twitch.helix.resources.streams import StreamNotFound

from .constants import *
from .exceptions import CommandError, InvalidGameError
...
def text_channels_alt(self):
    ...
    ret = [
        ...
        if c.category_id == self.id and isinstance(c, discord.TextChannel)
    ]
    for c in ret:
...
class Response(object):
    def __init__(
        ...
        self.delete_with_expire = delete_with_expire
...
class R6Bot(discord.Client):
    def __init__(self):
        ...
        self.helix = twitch.Helix(TWITCH_CREDS, use_cache=False)
        self.v5_api = twitch.V5(TWITCH_CREDS, use_cache=False)

        self.twitAPI = TwitterAPI(
            TWITTER_CON_KEY, TWITTER_CON_SECRET, TWITTER_TOKEN_KEY, TWITTER_TOKEN_SECRET
        )
        ...
        self.extra_settables = load_json("settables.json")
        ...
        self.muted_dict = {
            int(key): value for key, value in load_json("muted.json").items()
        }
        self.channel_bans = {
            int(key): {int(k): v for k, v in value.items()} if value else value
            for key, value in load_json("channel_banned.json").items()
        }
        self.cban_role_aliases = {
            key: int(value) for key, value in load_json("cbanrolealiases.json").items()
        }
        ...
        self.valid_other = VALID_OTHER_PRESET + self.extra_settables
        self.valid_strings = VALID_CHANNEL_STRINGS + self.valid_other
        self.server_status = {
            "PC": "online",
            "XBOXONE": "online",
            "PS4": "online",
        ...
        self.role_ping_toggle = {"server": None, "game": None, "esports": None}
        self.executor = concurrent.futures.ThreadPoolExecutor(max_workers=3)

        # Used to make RESTful calls using the User Token.
        self.user_http = HTTPClient(
        ...
        self.disable_auto_kick = False

        # I separated these out from Constants because I wanted to ensure they could be easily found and changed.
        self.intro_msg = "Welcome to the official Rainbow6 Discord server, make sure to read <#{rules}>! You will need a role in order to chat and join voice channels. To obtain a role *(take note of the exclamation mark prefix)*:```!pc\n!xbox\n!ps4``` If you happen to run a Youtube or Twitch  channel w/ over 15k followers or work at Ubisoft, dm me (the bot) about it and the admins will get you set up with a fancy role!".format(
            rules=CHANS["rules"]
        )
        self.dm_msg = "Hello and welcome to the official Rainbow6 Discord!\nPlease take a moment to review the rules in <#{rules}> and don't forget to assign yourself a role in <#{roleswap}> as you cannot use the text / voice channels until you do, if you have any further questions, simply message this bot back to send a mod mail to the server staff!".format(
            rules=CHANS["rules"], roleswap=CHANS["roleswap"]
        )

        # scheduler garbage
        self.scheduler = AsyncIOScheduler()
        self.scheduler.add_job(
            self.backup_messages_log, "cron", id="backup_msgs", minute="*/10"
        )
        self.scheduler.add_job(
            self.backup_modmail_logs, "cron", id="backup_modmail", minute="*/10"
        )
        self.scheduler.add_job(
            self.get_siege_server_status,
            "cron",
            id="get_siege_server_status",
            second="*/10",
        )
        self.scheduler.add_job(
            self.lfg_channel_informer, "cron", id="lfg_channel_informer", minute="*/3",
        )
        # self.scheduler.add_job(self.do_server_summary_month, 'cron', id='month_summary', day='1', hour='15')
        self.scheduler.add_job(
            self.remind_for_roles,
            "cron",
            id="remind_for_roles",
            day="1-30/3",
            hour="19",
    ...
    async def lfg_channel_informer(self):
        category = self.get_channel(690123534017495070)
        for channel in category.channels:
            has_recent_msg = await channel.history(limit=15).get(
                author__id=self.user.id, content=MSGS["lfg_awareness"]
            )
    ...
    async def queue_timed_ban_role(self, sec_time, user, timed_role, role_id, user_id):
        ...
        if self.channel_bans[role_id][user_id]:
            datetime_timestamp = datetime.fromtimestamp(
                self.channel_bans[role_id][user_id]
            )
            if datetime.utcnow() < datetime_timestamp:
                return
                print(
                    "Mute extended for {} for {} more seconds".format(
                        user.name if user != None else user_id,
                        (datetime_timestamp - datetime.utcnow()).total_seconds(),
                    )
                )
        else:
            return
        ...
        try:
            if not user or isinstance(user, discord.User):
                user = discord.utils.get(self.guilds, id=SERVERS["main"]).get_member(
                    user_id
                )
                if not user:
                    return
            ...
            if not timed_role:
                timed_role = discord.utils.get(
                    discord.utils.get(self.guilds, id=SERVERS["main"]).roles, id=role_id
                )
            if timed_role in user.roles:
                if (
                    not ROLES["staff"] in [role.id for role in user.roles]
                    and not user.bot
                ):
                    await user.remove_roles(timed_role, atomic=True)
        except:
            traceback.print_exc()
        ...
        try:
            ...
            if timed_role in user.roles:
                if (
                    ...
                    await user.remove_roles(timed_role, atomic=True)
                em = discord.Embed(
                    colour=discord.Colour(0xFFD800),
                    description=MUTED_MESSAGES["timed_over"].format(
                        roles=CHANS["roleswap"],
                        rules=CHANS["rules"],
                        muted=CHANS["muted"],
                    ),
                )
                await self.safe_send_message(user, embed=em)
                target_channel = await self.get_or_create_modmail_channel(
                    user.id, creation_state=None
                )
                if not self.use_new_modmail:
                    if target_channel:
                        channel_list = [
                            self.get_channel(CHANS["modmail"]),
                            target_channel,
                        ]
                    else:
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            chan,
                            content=MSGS["modmailaction"].format(
                                action_log_id=CHANS["actions"],
                                username=user.mention,
                                id=user.id,
                                reason="as they were unmuted",
                            ),
                        )
                else:
                    if target_channel:
    ...
    async def generate_streaming_embed(self, resp, streamer):
        iter_resp = iter(resp)
        user_data = next(iter_resp)
        stream_data = next(iter_resp)

        em = discord.Embed(
            colour=discord.Colour(0x56D696),
            description=stream_data.get("title"),
            timestamp=datetime.strptime(
                stream_data.get("started_at"), "%Y-%m-%dT%H:%M:%SZ"
            ),
        )
        prof_pic = user_data.get("profile_image_url")
        async with aiohttp.ClientSession() as sess:
            async with sess.get(
                stream_data.get("thumbnail_url").format(width=1280, height=720)
            ) as r:
                data = await r.read()
        ...
        em.set_footer(text="Language: {}".format(stream_data.get("language").upper()))

        em.add_field(name="Status", value="LIVE", inline=True)
        em.add_field(name="Viewers", value=stream_data.get("viewer_count"), inline=True)
        return em
    ...
    async def backup_modmail_log(self, user_id):
        if user_id in self.mod_mail_db:
            try:
                write_json(
                    "modmail_logs/%s.json" % str(user_id), self.mod_mail_db[user_id]
                )
            except:
                pass
    ...
    async def backup_persistent_data(self):
        ...
        def save_jsons():
            ...
            write_json("settables.json", self.extra_settables)
    ...
    async def mod_mail_reminders(self):
        while True:
            ...
            while ticker != 1800:
                ...
                if not [
                    member_id
                    for member_id in self.mod_mail_db
                    if not self.mod_mail_db[member_id]["answered"]
                ]:
                    ticker = 0
                else:
                    ticker += 1
                ...
                async for lmsg in self.get_channel(CHANS["staff"]).history(limit=1):
                    if self.last_modmail_msg and lmsg.id == self.last_modmail_msg.id:
                        await self.safe_edit_message(
                            lmsg,
                            content="There are **{}** unread items in the mod mail queue that're over a half hour old! Either run `!mmqueue` to see them and reply or mark them read using `!markread`!".format(
                                len(
                                    [
                                        member_id
                                        for member_id in self.mod_mail_db
                        ...
                        self.last_modmail_msg = await self.safe_send_message(
                            self.get_channel(CHANS["staff"]),
                            content="There are **{}** unread items in the mod mail queue that're over a half hour old! Either run `!mmqueue` to see them and reply or mark them read using `!markread`!".format(
                                len(
                                    [
                                        member_id
            ...
            try:
                for handle in self.twitter_watch_list:
                    ...
                    for item in reversed(list(r.get_iterator())):
                        ...
                        if (
                            "full_text" in item
                            and item["id"] > self.twitter_id_cache[handle]
                        ):
                            if self.twitter_debug:
                                print("new tweet found!")
                            if (
                                item["in_reply_to_screen_name"]
                                and item["in_reply_to_screen_name"] != handle
                            ):
                                if self.twitter_debug:
                                    print(
                                        "tweet is a reply to another, not in thread. skipping..."
                                    )
                                continue
                            if self.twitter_debug:
                                print("posting tweet by " + handle)
                            if item["id"] > highest_id:
                                highest_id = item["id"]
                            embed = await self.generate_new_tweet_embed(item)
                            await self.safe_send_message(
                                self.get_channel(CHANS["tweets"]), embed=embed
                            )
                    self.twitter_id_cache[handle] = highest_id
    ...
    async def generate_new_tweet_embed(self, resp):
        ...
        if "media" in resp["entities"]:
            if len(resp["entities"]["media"]) == 1:
                for media in resp["entities"]["media"]:
                    if not final_text:
                        final_text = clean_string(resp["full_text"]).replace(
                            media["url"], ""
                        )
                    else:
        ...
        if "urls" in resp["entities"]:
            for url in resp["entities"]["urls"]:
                if (
                    resp["is_quote_status"]
                    and url["url"] == resp["quoted_status_permalink"]["url"]
                ):
                    if not final_text:
                        final_text = clean_string(resp["full_text"]).replace(
                            url["url"], ""
        ...
        em = discord.Embed(
            colour=discord.Colour(0x00ACED), description=final_text, timestamp=date_time
        )
        if image_url:
            em.set_image(url=image_url)
        em.set_author(
            name=resp["user"]["screen_name"],
            url="https://twitter.com/{}/status/{}".format(
                resp["user"]["screen_name"], resp["id"]
            ),
            icon_url=resp["user"]["profile_image_url_https"],
        )
        if resp["is_quote_status"]:
            em.add_field(
                name="~~                    ~~",
                value=clean_string(
                    f'→*[**`@{resp["quoted_status"]["user"]["screen_name"]}`**]({resp["quoted_status_permalink"]["expanded"]}):*\n```{resp["quoted_status"]["full_text"]}```'
                ),
                inline=False,
            )
        return em
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.get(R6_STATUS_URL) as r:
                ...
                for platform_data in data:
                    ...
                    if self.debug_status_check:
                        print(f"[{platform}] {status} / {maintenance}")
                    if maintenance:
    ...
    async def check_twitch_streams(self):
        ...
        def get_twitch_game_data(game_name="Tom Clancy's Rainbow Six: Siege"):
            return self.helix.game(name=game_name).id

        def get_twitch_stream_data(twitch_handle):
            user_obj = self.helix.user(twitch_handle)
            if not user_obj:
                print(
                    f"Twitch username fucked, removing to prevent ratelimits... {twitch_handle}"
                )
                return None
            if not user_obj.stream:
                print(user_obj)
                return None
            stream_data = user_obj.stream.data
            return (user_obj.data, stream_data)

        executor = concurrent.futures.ThreadPoolExecutor(max_workers=3)
        loop = asyncio.get_event_loop()

        if not self.twitch_game_id:
            future = loop.run_in_executor(executor, get_twitch_game_data)

            self.twitch_game_id = await future
        ...
        try:
            await self.get_channel(414812315729526784).purge(limit=100, check=is_me)
        except:
            async for entry in self.get_channel(414812315729526784).history(
                limit=10000
            ):
                if entry.author == self.user:
                    await self.safe_delete_message(entry)
        ...
        try:
            await self.get_channel(675524137573154861).purge(limit=100, check=is_me)
        except:
            async for entry in self.get_channel(675524137573154861).history(
                limit=10000
            ):
                if entry.author == self.user:
                    await self.safe_delete_message(entry)
        ...
        while True:
            for streamer in self.twitch_watch_list[:]:
                future = loop.run_in_executor(
                    executor, get_twitch_stream_data, streamer
                )
                try:
                    resp = await future

                    if not resp:
                        self.twitch_watch_list.remove(streamer)
                        write_json("twitchwatchlist.json", self.twitch_watch_list)
                        continue
                    ...
                    if stream_data.get("game_id") != self.twitch_game_id:
                        raise InvalidGameError(
                            "Twitch Game ID is not matching only approved game"
                        )

                    if streamer in ["Rainbow6", "Rainbow6Bravo"]:
                        target_channel = 675524137573154861
                    elif streamer in self.proleague_streams:
                        target_channel = 414812315729526784
                    else:
                        target_channel = CHANS["twitch"]
                    if streamer not in self.twitch_is_live:
                        if self.twitch_debug:
                            print("Creating new embed for user %s" % streamer)
                        self.twitch_is_live[streamer] = {
                            "detected_start": datetime.utcnow(),
                            "view_count_updated": datetime.utcnow(),
                            "offline_cooldown": False,
                            "embedded_object": await self.generate_streaming_embed(
                                resp, streamer
                            ),
                            "message": None,
                        }
                        self.twitch_is_live[streamer][
                            ...
                            self.get_channel(target_channel),
                            embed=self.twitch_is_live[streamer]["embedded_object"],
                            file=discord.File(
                                "/home/bots/r6botrw/img.png", filename="img.png"
                            ),
                        )
                    else:
                        if (
                            datetime.utcnow() - timedelta(minutes=30)
                            > self.twitch_is_live[streamer]["detected_start"]
                        ):
                            if self.twitch_debug:
                                print("Recreating new embed for user %s" % streamer)
                            await self.safe_delete_message(
                                self.twitch_is_live[streamer]["message"]
                            )
                            self.twitch_is_live[streamer] = {
                                "detected_start": datetime.utcnow(),
                                "view_count_updated": datetime.utcnow(),
                            ...
                            > self.twitch_is_live[streamer]["view_count_updated"]
                        ):
                            if self.twitch_debug:
                                print(
                                    "Updating embeds view count for user %s" % streamer
                                )
                            self.twitch_is_live[streamer][
                                "embedded_object"
                            ] = await self.generate_streaming_embed(resp, streamer)
                            self.twitch_is_live[streamer][
                                "view_count_updated"
                            ] = datetime.utcnow()
                            await self.safe_edit_message(
                ...
                except (StreamNotFound, InvalidGameError):
                    if (
                        streamer in self.twitch_is_live
                        and not self.twitch_is_live[streamer]["offline_cooldown"]
                    ):
                        # if streamer in self.proleague_streams:
                        if self.twitch_debug:
                            print("User %s detected offline, deleting embed" % streamer)
                        await self.safe_delete_message(
                            self.twitch_is_live[streamer]["message"]
                        )
                        ...
                        # else:
                        #     if self.twitch_debug: print('User %s detected offline, deleting embed' % streamer)
    ...
    def load_channel_logs(self):
        ...
        for root, dirs, files in os.walk("channel_logs", topdown=False):
            for file in files:
                ...
                try:
                    channel_logs[int(file[:-5])] = {
                        int(key): value for key, value in load_json(fileroute).items()
                    }
                except:
                    pass
        final_dict = {}
    ...
    def load_modmail_logs(self):
        if not self.has_transitioned_mmdb:
            return {
                int(key): value
                for key, value in load_json("modmaildbbackup.json").items()
            }
        modmail_logs = {}
        for root, dirs, files in os.walk("modmail_logs", topdown=False):
            for file in files:
                fileroute = os.path.join(root, file)
                try:
                    modmail_logs[int(file[:-5])] = {
                        key: value for key, value in load_json(fileroute).items()
                    }
                except:
                    # traceback.print_exc()
    ...
    async def do_search(self, guild_id, **kwargs):
        ...
        return await self.user_http.request(
    ...
    async def get_or_create_modmail_channel(self, user_id, creation_state="new"):
        ...
        category = discord.utils.get(
            self.get_all_channels(), id=CATS[f"mm{creation_state}"]
        )
        user = discord.utils.get(category.guild.members, id=user_id)
        if not user:
            user = discord.utils.get(
                discord.utils.get(self.guilds, id=SERVERS["ban"]).members, id=user_id
            )
            if not user:
        ...
        if user_id in self.mod_mail_db:
            if any(
                [
                    message["modreply"]
                    for message in self.mod_mail_db[user_id]["messages"].values()
                ]
            ):
                symbol = MODMAIL_SYMBOLS["reply"]
                most_recent_timestamp = max(
                    self.mod_mail_db[user_id]["messages"].keys()
                )
                ...
                if self.mod_mail_db[user_id]["messages"][most_recent_timestamp][
                    "modreply"
                ]:
                    symbol = MODMAIL_SYMBOLS["wfr"]
        try:
            channel = await category.guild.create_text_channel(
                f"⦗{symbol}⦘{user.name}#{user.discriminator}",
                category=category,
                topic=str(user_id),
            )
        except discord.HTTPException as e:
            channel = await category.guild.create_text_channel(
                f"⦗{symbol}⦘{user.id}", category=category, topic=str(user_id)
            )
    ...
    async def transition_modmail_channels(self):
        await self.wait_until_really_ready()
        if self.new_modmail_debug:
            print("attempting to transition modmail channels")
        if self.is_sorting_channels:
            if self.new_modmail_debug:
                print("already sorting, skipping...")
            return
        ...
        try:
            while any(value == False for value in modmail_category_cleared.values()):
                ...
                for category_id in list(reversed(CAT_TRANSITION_ORDER)):
                    category = discord.utils.get(
                        self.get_all_channels(), id=category_id
                    )
                    if self.new_modmail_debug:
                        print(f"On Cat ID {category_id} Name {category.name}")
                    channels = category.text_channels_alt
                    if len(channels) > 45:
                        if self.new_modmail_debug:
                            print(
                                f"More than 45 channels, currently at {len(channels)}"
                            )
                        transition_category = None
                        if CAT_TRANSITION[category_id] != "delete":
                            transition_category = discord.utils.get(
                                self.get_all_channels(), id=CAT_TRANSITION[category_id]
                            )
                            if self.new_modmail_debug:
                                print(
                                    f"has transition category, id is {transition_category.id} name {transition_category.name}"
                                )
                        try:
                            while len(channels) > 45:
                                ...
                                if self.new_modmail_debug:
                                    print(
                                        f"attempting to move channel id {channel.id} name {channel.name}"
                                    )
                                if transition_category:
                                    if self.new_modmail_debug:
                                        print(f"it has a transition category")
                                    ...
                                    if self.new_modmail_debug:
                                        print(f"its being deleted")
                                    await channel.delete()
                                if self.new_modmail_debug:
                                    print(f"sleeping between movements")
                        ...
                        if self.new_modmail_debug:
                            print(f"modmail category cleared!")
                        modmail_category_cleared[category_id] = True
                    if self.new_modmail_debug:
                        print(f"sleeping between categories")
                    await asyncio.sleep(0.5)
        except:
            traceback.print_exc()
            pass
    ...
    async def populate_modmail_channel(self, user, channel, n_of_messages=20):
        ...
        messages = collections.OrderedDict(
            sorted(self.mod_mail_db[user.id]["messages"].items(), reverse=True)
        )
        messages = collections.OrderedDict(
            reversed(list(islice(messages.items(), 0, n_of_messages)))
        )
        for timestamp, message in messages.items():
            if message["modreply"]:
                msg_user = discord.utils.get(
                    channel.guild.members, id=message["modreply"]
                )
                if not msg_user:
                    if message["modreply"] not in cached_modreply_users:
                        cached_modreply_users[
                            message["modreply"]
                        ] = await self.fetch_user(message["modreply"])
            ...
            em = discord.Embed(
                description=f"**{msg_user.mention} - {msg_user.name}#{msg_user.discriminator}**",
                colour=msg_user.color,
    ...
    async def on_ready(self):
        ...
        new_roles = [
            role
            for role in discord.utils.get(self.guilds, id=SERVERS["main"]).roles
            if role.name.startswith("Ban") and role.id not in self.channel_bans
        ]
        if new_roles:
            print("Found %s new roles!" % len(new_roles))
            for role in new_roles:
                self.channel_bans[role.id] = {
                    member.id: None
                    for member in discord.utils.get(
                        self.guilds, id=SERVERS["main"]
                    ).members
        ...
        for member_id in [
            ...
            if any(
                [
                    message["modreply"]
                    for message in self.mod_mail_db[member_id]["messages"].values()
                ]
            ):
                await self.get_or_create_modmail_channel(
                    member_id, creation_state="reply"
                )
            else:
                await self.get_or_create_modmail_channel(
                    member_id, creation_state="new"
                )
        ...
        temp_dict = [
            user_id
            for user_id in self.muted_dict.keys()
            if discord.utils.find(lambda u: u.user.id == user_id, ban_list)
        ]
        ...
        temp_dict = dict(
            {
                user_id: timestamp
                for user_id, timestamp in self.muted_dict.items()
                if timestamp or target_server.get_member(user_id)
            }
        )
        for user_id, timestamp in temp_dict.items():
            ...
            if timestamp:
                ...
                if datetime.utcnow() < datetime_timestamp:
                    ...
                    print(
                        "Queueing serialized mute for {} for {} seconds".format(
                            user.name if user != None else user_id,
                            (datetime_timestamp - datetime.utcnow()).total_seconds(),
                        )
                    )
                    ...
                    asyncio.ensure_future(
                        self.queue_timed_mute(0, user, mutedrole, user_id)
                    )
                    print(
                        "Serialized mute period has passed for {}".format(
                            user.name if user != None else user_id
                        )
                    )
                    continue
                ...
                print(
                    "Queueing serialized nontimed mute for {}".format(
                        user.name if user != None else user_id,
                    )
                )
            if user:
                        ...
                        roles = set(
                            (
                                [
                                    ...
                                    if role.id not in UNPROTECTED_ROLES
                                ]
                                + [mutedrole]
                            )
                        )
        ...
        temp_dict = dict(
            {
                ...
                for role_id, user_blob in self.channel_bans.items()
                if user_blob
            }
        )
        for role_id, user_blob in temp_dict.items():
            ...
            for user_id, timestamp in user_blob.items():
                ...
                if timestamp:
                    ...
                    if datetime.utcnow() < datetime_timestamp:
                        ...
                        print(
                            "Queueing serialized cban for {} for {} seconds".format(
                                user.name if user != None else user_id,
                                (
                                    datetime_timestamp - datetime.utcnow()
                                ).total_seconds(),
                        ...
                        asyncio.ensure_future(
                            self.queue_timed_ban_role(
                                0, user, cban_role, role_id, user_id
                            )
                        )
                        print(
                            "Serialized cban period has passed for {}".format(
                                user.name if user != None else user_id
                            )
                        )
                        continue
                    ...
                    print(
                        "Queueing serialized nontimed cban for {}".format(
                            user.name if user != None else user_id,
                        )
                    )
                if user:
        ...
        muted_coffee_filter = [
            member
            for member in discord.utils.get(self.guilds, id=SERVERS["main"]).members
            if mutedrole in member.roles and member.id not in self.muted_dict
        ]
        ...
        await self.safe_send_message(
            self.get_channel(CHANS["staff"]), content="I have just been rebooted!"
        )

        print("\n~")

    async def _wait_delete_msg(self, message, prev_msg, after):
        ...
        *,
        ...
        try:
            ...
            if msg and expire_in:
                asyncio.ensure_future(
                    self._wait_delete_msg(msg, delete_with_expire, expire_in)
                )
            ...
            if (
                not self_called
                and hasattr(dest, "category")
                and hasattr(dest.category, "id")
                and dest.category.id in CATS.values()
                and dest.id
                in [
                    value.id
                    for value in self.cached_modmail_channels.values()
                    if hasattr(value, "id")
                ]
            ):
                if dest.topic:
                    del self.cached_modmail_channels[int(dest.topic)]
                    new_chan = await self.get_or_create_modmail_channel(
                        dest.topic, creation_state="new"
                    )
                ...
                print(
                    f"Bugged Modmail Cache:\n\tDest:{dest.topic if dest.topic else ''}{dest.name if dest.name else ''}\n{ {key: val for key, val in self.cached_modmail_channels.items() if not val} }"
                )
            if not quiet:
                print(
    ...
    async def safe_edit_channel(self, channel, *, quiet=False, **options):
        ...
        try:
            ...
            if (
                ...
                and hasattr(channel.category, "id")
                and channel.category.id in CATS.values()
                and channel.id
                in [
                    value.id
                    for value in self.cached_modmail_channels.values()
                ...
                if channel.topic:
                    ...
                    new_chan = await self.get_or_create_modmail_channel(
                        channel.topic, creation_state="new"
                    )
                    chan = await new_chan.edit(**options)
                    print("Handled 404 in Modmail")
            if not quiet:
                print(
                    f'Warning: Cannot edit channel "{channel.name}:{channel.id}", channel not found'
                )
        finally:
            return chan
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if (
                ...
                or [
                    ...
                    if role.id in [ROLES["staff"]]
                ]
                or (
                    is_tag
                    and (
                        self.tags[is_tag][0] == "unrestricted_eval"
                        or [
                            role
                            for role in orig_msg.author.roles
                            if role.id in [ROLES["tagmaster"], ROLES["staff"]]
                        ]
                    )
                )
            ):
    ...
    async def cmd_alias(self, guild, raw_leftover_args):
        ...
        if not raw_leftover_args or len(raw_leftover_args) < 2:
            newline = "\n"
            return Response(
                f'I know the following aliases:{chr(10)}{chr(10)}{newline.join([f"- {alias} for <@&{role_id}>" for alias, role_id in self.cban_role_aliases.items()])}'
            )
    ...
    async def cmd_inv(self, message, channel, author):
        ...
        if (
            ...
            and author.voice.channel.name.lower().startswith(("[c", "[r", "[u", "[tts"))
        ):
            invite = await author.voice.channel.create_invite(max_uses=4, max_age=600)
            await self.safe_send_message(author, content=invite)
            await self.safe_delete_message(message)
        else:
            return Response(
                "You must be in a LFG Voice Channel to use this command!",
                reply=True,
                delete_after=10,
            )
    ...
    async def cmd_lfg(self, message, channel, author, raw_leftover_args):
        ...
        """
        if channel.id not in [290537712644128768] + LFG_CHANS:
            return Response(
                "This command can only be used in the LFG text channels!",
                reply=True,
                delete_after=5,
            )
        ...
        if (
            ...
            if raw_leftover_args:
                ...
                await self.safe_send_message(
                    channel,
                    content=f"Join {' + '.join([user.mention for user in author.voice.channel.members])} in {author.voice.channel.name} via {invite}\n```\n{' '.join((message.clean_content.strip().split())[1:])}\n```",
                )

                raw_leftover_args = [
                    pattern.sub("", item) for item in raw_leftover_args
                ]
                await self.cmd_set(
                    message, channel, author, raw_leftover_args, from_lfg=True
                )
            else:
                await self.safe_send_message(
                    channel,
                    content=f"Join {author.mention} in {author.voice.channel.name} via {invite}",
                )
        else:
            return Response(
                "You must be in a LFG Voice Channel to use this command!",
    ...
    async def cmd_set(
        self, message, channel, author, raw_leftover_args, from_lfg=False
    ):
        """
        Usage: {command_prefix}set <flags>
        If the user is in a voice channel in a LFG category, it will set additional key words on the name of that channel.
        If the user who sets the name of a voice channel leaves, the channel auto clears the name.

        Simply putting "help" will list all possible key words as well.
        """
        if (
            author.voice
            and author.voice.channel
            ...
            chan_name = re.search(r"(.*)(])", author.voice.channel.name)
            if not chan_name:
                return
            chan_name = chan_name[0]

            if not raw_leftover_args and author.id in self.vc_name_locks:
                await author.voice.channel.edit(name=chan_name)
                self.vc_name_locks.pop(author.id, None)
                await self.safe_delete_message(message)
                return Response(
                    ":thumbsup:", reply=True, delete_after=5, delete_invoking=True
                )
            elif (not raw_leftover_args and author.id not in self.vc_name_locks) or (
                raw_leftover_args and raw_leftover_args[0].lower() == "help"
            ):
                if channel.id in [278986547187941377, 290537712644128768]:
                    nl = "\n"
                    return Response(
                        f"I currently support the following key words:\nRegions```\n{nl.join(sorted(VALID_REGIONS))}```\nPlatforms```\n{nl.join(sorted(VALID_PLATFORMS))}```\nRanks```\n{nl.join(VALID_RANKS)}```\nVarious Other Flags```\n{nl.join(sorted(self.valid_other))}```\n\nTry Running `!set chill PC` once in a LFG VC to see the effect!"
                    )
                return Response(
                    "Due to how long the help command is, we're asking for users to temporarily head over to <#278986547187941377> before you run it. DONT WORRY, the `!set` command works perfectly there too!",
                    reply=True,
                    delete_after=5,
                    delete_invoking=True,
                )
            elif author.voice.channel.id not in list(self.vc_name_locks.values()) or (
                author.id in self.vc_name_locks
                and self.vc_name_locks[author.id] == author.voice.channel.id
            ):

                ordered_strings = [[], [], [], []]
                lookup_dict = {item.lower(): item for item in self.valid_strings}

                for arg in raw_leftover_args:
                    if arg.lower() in lookup_dict:
                        if lookup_dict[arg.lower()] in VALID_RANKS:
                            if not ordered_strings[0]:
                                ordered_strings[0].append(lookup_dict[arg.lower()])
                        elif lookup_dict[arg.lower()] in VALID_REGIONS:
                            if not ordered_strings[1]:
                                ordered_strings[1].append(lookup_dict[arg.lower()])
                        elif lookup_dict[arg.lower()] in VALID_PLATFORMS:
                            if not ordered_strings[2]:
                                ordered_strings[2].append(lookup_dict[arg.lower()])
                        elif lookup_dict[arg.lower()] in self.valid_other:
                            ordered_strings[3].append(lookup_dict[arg.lower()])

                final_strings = [" | ".join(item) for item in ordered_strings if item]
                await author.voice.channel.edit(
                    name=f"{chan_name} {' | '.join(final_strings)}"
                )
                ...
                return Response(
                    "The Voice Channel you're trying to set key words for already has key words set by someone else!",
                    reply=True,
                    delete_after=5,
                    delete_invoking=True,
                )
        ...
        return Response(
            "You're not currently in an LFG Voice Channel, join one to use this command",
            reply=True,
            delete_after=5,
            delete_invoking=True,
        )
    ...
    async def cmd_shrink(self, message, channel, author):
        ...
        if (
            ...
            if author.id not in self.vc_num_locks:
                if (
                    author.voice.channel.id not in list(self.vc_num_locks.values())
                    and len(author.voice.channel.members) > 1
                ):
                    await author.voice.channel.edit(
                        user_limit=len(author.voice.channel.members)
                    )
                    self.vc_num_locks[author.id] = author.voice.channel.id
                    await self.safe_delete_message(message)
                    return Response(
                        ":thumbsup:", reply=True, delete_after=3, delete_invoking=True
                    )
            else:
    ...
    async def cmd_toggleproleague(self, author, twitch_channel):
        ...
        if twitch_channel not in self.proleague_streams:
            ...
            self.proleague_streams = [
                item for item in self.proleague_streams if item != twitch_channel
            ]
    ...
    async def cmd_clear(self, message, author, channel):
        ...
        if mod_check:
            ...
            author_roles = []
        if not ROLES["staff"] in [role.id for role in author.roles]:
            await author.edit(roles=author_roles)
        return Response(
            "I've removed all platform roles from you!",
            reply=True,
            delete_after=15,
            delete_invoking=True,
        )
            ...
            if switch in ["+", "add"]:
                ...
                try:
                    plain_sorted = sorted(
                        [
                            ...
                            if not self.tags[tag][0]
                            or "unrestricted_eval" in self.tags[tag][0].split()
                        ],
                        key=str.lower,
                    )
                    ...
                    is_staff = any(
                        [
                            ...
                            if role.id in [ROLES["staff"], ROLES["tagmaster"]]
                        ]
                    )
                    for tag in [
                        tag
                        for tag, value in self.tags.items()
                        if value[0] and "unrestricted_eval" not in value[0].split()
                    ]:
                        tag_flag_list = []
                        channel_list = []
                        should_ignore = False
                        ...
                        for tag_flag in self.tags[tag][0].split():
                            if (
                                not is_staff and tag_flag in ["restrict", "eval"]
                            ) or should_ignore:
                                should_ignore = True
                                continue
                            if tag_flag.isdigit() and guild.get_channel(int(tag_flag)):
                                ...
                                if isinstance(chan, discord.CategoryChannel):
                                    channel_list = channel_list + [
                                        cat_chan
                                        for cat_chan in chan.text_channels
                                        if cat_chan.permissions_for(
                                            author
                                        ).read_messages
                                    ]
                                else:
                        ...
                        if key in unique_tag_flags:
                            unique_tag_flags[key].append(
                                (tag, tag_flag_list, channel_list)
                            )
                        else:
                        ...
                        for tag_list in unique_tag_flags.values():
                            if tag_list[0][1]:
                                tag_list_lines.append(
                                    f"**Tag Flags:** `{', '.join(tag_list[0][1])}`"
                                )
                            if tag_list[0][2]:
                                tag_list_lines.append(
                                    f"**In Channels:** {', '.join([chan.mention for chan in tag_list[0][2]])}"
                                )
                            special_sorted = sorted(
                                [tag[0] for tag in tag_list], key=str.lower
                            )
                            tag_list_lines.append(special_sorted[0])
                            for elem in special_sorted[1:]:
                                if len(tag_list_lines[-1]) + len(elem) < 70:
                                    tag_list_lines[-1] = (
                                        tag_list_lines[-1] + ", " + elem
                                    )
                                else:
                                    tag_list_lines.append(elem)
                ...
                if [role for role in author.roles if role.id in [ROLES["staff"]]]:
                    for user in mentions:
                        ...
                        return Response(
                            "User `{}` was blacklisted".format(
                                clean_bad_pings(user.name)
                            ),
                            delete_after=20,
                            delete_invoking=True,
                        )
                ...
                if [role for role in author.roles if role.id in [ROLES["staff"]]]:
                    try:
                        ...
                        return Response(
                            'Tag "%s" removed'
                            % clean_bad_pings(" ".join(leftover_args)),
                            delete_after=10,
                            delete_invoking=True,
                        )
                    except:
                    ...
                    if self.tags[tag][0]:
                        ...
                        for item in tag_flags:
                            if item.isdigit() and guild.get_channel(int(item)):
                                ...
                                if isinstance(chan, discord.CategoryChannel):
                                    acceptable_chans = acceptable_chans + [
                                        cat_chan.id for cat_chan in chan.text_channels
                                    ]
                                else:
                                    acceptable_chans.append(
                                        (guild.get_channel(int(item))).id
                                    )
                        if (
                            channel.id not in acceptable_chans
                            and len(acceptable_chans) > 0
                        ):
                            return Response(
                                f'Tag cannot be used outside of {", ".join([f"<#{chan}>" for chan in acceptable_chans])}',
                                delete_after=20,
                            )
                        ...
                        if "unrestricted_eval" in tag_flags:
                            ...
                            if resp:
                                await self.safe_send_message(
                                    channel, content=clean_bad_pings(resp)
                                )
                            return
                        elif "restrict" in tag_flags and not [
                            role
                            for role in author.roles
                            if role.id in [ROLES["staff"], ROLES["tagmaster"]]
                        ]:
    ...
    async def cmd_staff(self, message, channel, author, guild):
        ...
        """
        check_msg = await self.safe_send_message(
            channel,
            content="Please remember to *only* tag staff for things that **absolutely** require **immediate** attention and can only be addressed by server administration.\n\nFor non-immediate topics, *please* send a private message to <@!278980093102260225> to send mod-mail to the administration.\n\nPlease react if you still wish to ping the staff",
        )
        ...
        def check(reaction, user):
            return (
                user.id == author.id
                and reaction.message.id == check_msg.id
        ...
        if str(reac.emoji) == "✅":
            ...
            mention_msg = await self.safe_send_message(
                channel, content="<@&{}>".format(ROLES["staff"])
            )
            await asyncio.sleep(1)
    ...
    async def cmd_cmdinfo(self, author, command=None):
        ...
        if [role for role in author.roles if role.id in [ROLES["staff"]]]:
            if command:
                ...
                if cmd:
                    return Response(
                        "```\n{}```".format(textwrap.dedent(cmd.__doc__)).format(
                ...
                helpmsg += "```\n"
                helpmsg += "You can also use `{}help x` for more info about each command.".format(
                    self.prefix
                )
            ...
            if command:
                ...
                for att in dir(self):
                    if (
                        att.startswith("cmd_")
                        and att != "cmd_help"
                        and not hasattr(getattr(self, att), "mod_cmd")
                    ):
                        command_name = att.replace("cmd_", "").lower()
                        commands.append("{}{}".format(self.prefix, command_name))
    ...
    async def cmd_check(self, message, mentions, author, guild, leftover_args):
        ...
        if user.id in self.serious_d_blacklist:
            return Response(
                "%s is Serious Discussion blacklisted!" % clean_bad_pings(user.name)
            )
        else:
            return Response(
                "%s is not Serious Discussion blacklisted!" % clean_bad_pings(user.name)
            )

    @mods_only
    async def cmd_removesettable(self, author, settable):
        """
        Usage {command_prefix}removesettable <item>
        Removes the string from the list of misc settables
        """
        if settable in self.extra_settables:
            self.extra_settables.remove(settable)
            self.valid_other = VALID_OTHER_PRESET + self.extra_settables
            self.valid_strings = VALID_CHANNEL_STRINGS + self.valid_other
            write_json("settables.json", self.extra_settables)
            return Response(":thumbsup:")
        else:
            raise CommandError("ERROR: Word not in settables")

    @mods_only
    async def cmd_makesettable(self, author, settable):
        """
        Usage {command_prefix}makesettable <item>
        Adds the string from the list of misc settables
        """
        self.extra_settables.append(settable)
        self.valid_other = VALID_OTHER_PRESET + self.extra_settables
        self.valid_strings = VALID_CHANNEL_STRINGS + self.valid_other
        write_json("settables.json", self.extra_settables)
        return Response(":thumbsup:")
    ...
    async def cmd_testnewtweet(self, channel, twitter_id):
        ...
        def get_twitter_data(twitterid):
            return self.twitAPI.request(
                f"statuses/show/:{twitterid}", {"tweet_mode": "extended"}
            )
    ...
    async def cmd_markread(self, channel, author, guild, auth_id=None):
        ...
        if auth_id in self.mod_mail_db:
            ...
            channel = await self.get_or_create_modmail_channel(
                auth_id, creation_state=None
            )
            target_category = discord.utils.get(
                self.get_all_channels(), id=CATS[f"mmread"]
            )
            if channel and channel.category.id != target_category:
                await self.safe_edit_channel(
                    channel,
                    category=target_category,
                    name=f"⦗{MODMAIL_SYMBOLS['read']}{channel.name[2:]}",
                    sync_permissions=True,
                )
                asyncio.ensure_future(self.transition_modmail_channels())
            return Response(":thumbsup:")
    ...
    async def cmd_mmlogs(self, author, channel, guild, auth_id):
        ...
        message_dict = collections.OrderedDict(
            sorted(self.mod_mail_db[auth_id]["messages"].items(), reverse=True)
        )
        while True:
            ...
            quick_switch_dict = {
                "embed": discord.Embed(),
                "member_obj": await self.fetch_user(auth_id),
            }
            quick_switch_dict["embed"].set_author(
                name="{}({})".format(
                    quick_switch_dict["member_obj"].name,
                    quick_switch_dict["member_obj"].id,
                ),
                icon_url=quick_switch_dict["member_obj"].avatar_url,
            )
            od = collections.OrderedDict(
                islice(message_dict.items(), current_index, current_index + step)
            )
            od = collections.OrderedDict(reversed(list(od.items())))
            total_length = sum(
                [len(msg_dict["content"]) for timestamp, msg_dict in od.items()]
            )
            ...
            for timestamp, msg_dict in od.items():
                ...
                if msg_dict["modreply"] is not None:
                    try:
                        user = discord.utils.get(
                            guild.members, id=msg_dict["modreply"]
                        ).name
                    except:
                ...
                if sub_per_msg:
                    msg_dict["content"] = (
                        msg_dict["content"][: (1990 - sub_per_msg)] + "..."
                    )
                # if len(msg_dict['content']) > 1020:
                # msg_dict['content'] = msg_dict['content'][:1020] + '...'
                quick_switch_dict["embed"].add_field(
                    name="{} | *{}*".format(
                        user,
                        datetime.utcfromtimestamp(float(timestamp)).strftime(
                            "%H:%M %d.%m.%y"
                        ),
                    ),
                    value=msg_dict["content"],
                    inline=False,
                )
            if not current_msg:
                current_msg = await self.safe_send_message(
                    channel, embed=quick_switch_dict["embed"]
                )
            else:
                current_msg = await self.safe_edit_message(
                    current_msg, embed=quick_switch_dict["embed"]
                )
            ...
            if str(reac.emoji) == "ℹ":
                await self.safe_send_message(
                    current_msg.channel, content=quick_switch_dict["member_obj"].id
                )
    ...
    async def cmd_mmqueue(self, author, channel, guild):
        ...
        for member_id in unanswered_threads:
            quick_switch_dict[member_id] = {
                "embed": discord.Embed(),
                "member_obj": await self.fetch_user(member_id),
            }
            quick_switch_dict[member_id]["embed"].set_author(
                name="{}({})".format(
                    quick_switch_dict[member_id]["member_obj"].name,
                    quick_switch_dict[member_id]["member_obj"].id,
                ),
                icon_url=quick_switch_dict[member_id]["member_obj"].avatar_url,
            )
            od = collections.OrderedDict(
                sorted(self.mod_mail_db[member_id]["messages"].items(), reverse=True)
            )
            ...
            for timestamp, msg_dict in od.items():
                ...
                quick_switch_dict[member_id]["embed"].add_field(
                    name="{} | *{}*".format(
                        user,
                        datetime.utcfromtimestamp(float(timestamp)).strftime(
                            "%H:%M %d.%m.%y"
        ...
        while True:
            ...
            embed_object.set_footer(
                text="{} / {}".format(current_index + 1, len(loop_dict))
            )
            ...
            if not current_msg:
                ...
                current_msg = await self.safe_edit_message(
                    current_msg, embed=embed_object
                )
            ...
            try:
                ...
                return
            if str(reac.emoji) == "☑":
                if not self.mod_mail_db[loop_dict[current_index]["member_obj"].id][
                    "answered"
                ]:
                    self.mod_mail_db[loop_dict[current_index]["member_obj"].id][
                        "answered"
                    ] = True
                    channel = await self.get_or_create_modmail_channel(
                        loop_dict[current_index]["member_obj"].id, creation_state=None
                    )
                    target_category = discord.utils.get(
                        self.get_all_channels(), id=CATS[f"mmread"]
                    )
                    ...
                    if len(loop_dict) == 0:
                        ...
                        await self.safe_send_message(
                            current_msg.channel, content="Everything is answered!"
                        )
                        return
                    else:
                        await current_msg.remove_reaction(reac.emoji, user)
                ...
                await self.safe_send_message(
                    current_msg.channel,
                    content=loop_dict[current_index]["member_obj"].id,
                )
            elif str(reac.emoji) == "⬅" and current_index != 0:
                current_index -= 1
                await current_msg.remove_reaction(reac.emoji, user)
    ...
    @mods_only
    async def cmd_cr(
        self, message, channel, mentions, author, guild, raw_leftover_args
    ):
        """
        ...
        if not raw_leftover_args:
            return Response(
                doc_string(
                    inspect.getdoc(
                        getattr(
                            self, inspect.getframeinfo(inspect.currentframe()).function
                        )
                    ),
                    self.prefix,
                )
        ...
        if self.tags[tag][0]:
            ...
            if "unrestricted_eval" in tag_flags or "eval" in tag_flags:
                ...
                if not resp:
                    raise CommandError(
                        "Error: The tag you chose was an eval which does not return any text and therefore cannot be used in a canned response"
                    )
                tag_content = resp
    ...
    async def cmd_r(self, channel, author, guild, raw_leftover_args):
        ...
        try:
            ...
            raise CommandError(
                "This channel's topic was fucked with, please put JUST the ID of the user to modmail in the topic and ping rhino"
            )
        member = discord.utils.get(guild.members, id=auth_id)

        if not member:
            member = discord.utils.get(
                discord.utils.get(self.guilds, id=SERVERS["ban"]).members, id=auth_id
            )
            if not member:
                try:
                    ...
                    try:
                        member = await discord.utils.get(
                            self.guilds, id=SERVERS["ban"]
                        ).fetch_member(auth_id)
                    except:
                        pass
        if member:
            if raw_leftover_args[0] == "anon":
                ...
                if member.id in self.mod_mail_db:
                    self.mod_mail_db[member.id]["messages"][
                        "{}".format(datetime_to_utc_ts(datetime.now()))
                    ] = {
                        "content": "(ANON){}".format(msg_to_send),
                        "modreply": author.id,
                    }
                    self.mod_mail_db[member.id]["answered"] = True
                else:
                    self.mod_mail_db[member.id] = {
                        "answered": True,
                        "messages": {
                            "{}".format(datetime_to_utc_ts(datetime.now())): {
                                "modreply": author.id,
                ...
                em = discord.Embed(
                    description=f"**{author.mention}**", colour=author.color
                )
                ...
                if member.id in self.mod_mail_db:
                    self.mod_mail_db[member.id]["messages"][
                        "{}".format(datetime_to_utc_ts(datetime.now()))
                    ] = {"content": "{}".format(msg_to_send), "modreply": author.id}
                    self.mod_mail_db[member.id]["answered"] = True
                else:
                    self.mod_mail_db[member.id] = {
                        "answered": True,
            ...
            target_category = discord.utils.get(
                self.get_all_channels(), id=CATS[f"mmwfr"]
            )
            if channel.category.id != target_category:
                await self.safe_edit_channel(
                    channel,
                    category=target_category,
                    name=f"⦗{MODMAIL_SYMBOLS['wfr']}{channel.name[2:]}",
                    sync_permissions=True,
                )
                asyncio.ensure_future(self.transition_modmail_channels())
            ...
            jump_embed = (
                discord.Embed(
                    ...
                    description=f"[click to jump (if available)]({target_channel_msg.jump_url})",
                )
            ...
            await self.safe_send_message(
                self.get_channel(CHANS["modmail"]),
                content=f"{author.name} sent this to {member.mention} in new mod mail:```{msg_to_send}```",
                embed=jump_embed,
            )
        else:
    ...
    async def cmd_modmail(self, author, guild, raw_leftover_args):
        ...
        if member:
            if raw_leftover_args[0] == "anon":
                ...
                if not self.use_new_modmail:
                    await self.safe_send_message(
                        member, content="**Staff:** {}".format(msg_to_send)
                    )
                else:
                    await self.safe_send_message(member, embed=em)

                channel = await self.get_or_create_modmail_channel(
                    member.id, creation_state="wfr"
                )
                if member.id in self.mod_mail_db:
                    self.mod_mail_db[member.id]["messages"][
                        "{}".format(datetime_to_utc_ts(datetime.now()))
                ...
                if not self.use_new_modmail:
                    await self.safe_send_message(
                        member, content="**{}:** {}".format(author.name, msg_to_send)
                    )
                else:
                    await self.safe_send_message(member, embed=em)
            ...
            if not self.use_new_modmail:
                return Response(
                    ":thumbsup: Sent this to {}:```{}```".format(
                        member.name, msg_to_send
                    )
                )
            else:
    ...
    async def cmd_timeleft(self, author):
        ...
        """
        if (
            ROLES["muted"] in [role.id for role in author.roles]
            and self.muted_dict[author.id]
        ):
            return Response(
                "You will be unmuted in %s"
                % strfdelta(
                    datetime.fromtimestamp(self.muted_dict[author.id])
                    - datetime.utcnow()
                )
            )
        return
    ...
    async def cmd_unmute(self, message, guild, author, mentions, raw_leftover_args):
        ...
        if mentions:
            ...
            for item in temp:
                try:
                    user_to_append = await converter.convert(
                        message, self, item, discrim_required=True
                    )
        ...
        for user in users:
            try:
                ...
                write_json("muted.json", self.muted_dict)
                action_msg = await self.safe_send_message(
                    self.get_channel(CHANS["actions"]),
                    content=MSGS["action"].format(
                        username=user.name,
                        optional_content="",
                        discrim=user.discriminator,
                        id=user.id,
                        action="Unmuted user",
                        reason="Action taken by {}#{}".format(
                ...
                raise CommandError(
                    "Not enough permissions to mute user : {}".format(user.name)
                )
            except:
                traceback.print_exc()
                raise CommandError(
                    "Unable to mute user defined:\n{}\n".format(user.name)
                )
        return Response(":thumbsup:")

    @mods_only
    async def cmd_reason(
        self, message, guild, author, mentions, message_id, leftover_args
    ):
        """
        ...
        for msg_id in msg_ids:
            try:
                ...
                if author.id in self.last_actions:
                    ...
                    try:
                        ...
                        raise CommandError(
                            f"You do not have a message ID stored in the cache. The bot must have recently rebooted. Please use the ID of the message you'd like to provide a reason for."
                        )
            if msg.author.id != self.user.id:
                return Response(
                    f"I cannot edit other people's messages. Please speak to {msg.author.mention} if you'd like to change their reason"
                )
            ...
            edited_content[
                0
            ] = f"{edited_content[0]} (Taken by {author.name}#{author.discriminator})"
            edited_content = "\n".join(edited_content)
            bonus_content = (
                ""
                if not message.attachments
                else ", ".join([attch.url for attch in message.attachments])
            )
            edited_content = (
                f"```{edited_content}\nReason: {reason}\n```{bonus_content}"
            )
        ...
        for user in users:
            try:
                ...
                if (
                    ...
                    await user.edit(
                        roles=list(set(([role for role in user.roles] + [role])))
                    )
            except discord.Forbidden:
                raise CommandError(
                    "Not enough permissions to channel ban user : {}".format(user.name)
                )
            except:
                traceback.print_exc()
                raise CommandError(
                    "Unable to channel ban user defined:\n{}\n".format(user.name)
                )
        response = ":thumbsup:"
            ...
            if seconds_to_mute:
                ...
                action_msg = await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        discrim=user.discriminator,
                        id=user.id,
                        action="Applied {} channel ban for {}".format(
                            role.name[4:], " ".join(raw_leftover_args)
                        ),
                        reason="Action taken by {}#{}".format(
                            author.name, author.discriminator
                        ),
                ...
                em = discord.Embed(
                    ...
                    description=CBAN_MESSAGES["timed"].format(
                        cban_name=role.name[4:],
                        time=" ".join(raw_leftover_args),
                        rules=CHANS["rules"],
                    ),
                )
                await self.safe_send_message(user, embed=em)
                asyncio.ensure_future(
                    self.queue_timed_ban_role(
                        seconds_to_mute, user, role, role.id, user.id
                    )
                )
                ...
                action_msg = await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        discrim=user.discriminator,
                        id=user.id,
                        action="Applied {} channel ban forever".format(role.name[4:]),
                        reason="Action taken by {}#{}".format(
                            author.name, author.discriminator
                        ),
                    ),
                ...
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000),
                    description=CBAN_MESSAGES["plain"].format(
                        cban_name=role.name[4:], rules=CHANS["rules"]
                    ),
                )
                await self.safe_send_message(user, embed=em)
            target_channel = await self.get_or_create_modmail_channel(
                ...
                for chan in channel_list:
                    await self.safe_send_message(
                        ...
                        content=MSGS["modmailaction"].format(
                            ...
                            username=user.mention,
                            id=user.id,
                            reason=f"as they were assigned {role.name}",
                        ),
                    )
            else:
                if target_channel:
    ...
    @mods_only
    async def cmd_addr(
        self, message, channel, guild, author, mentions, raw_leftover_args
    ):
        """
        ...
        if not users:
            try:
                ...
                user_to_append = await converter.convert(
                    message, self, str(user_id), discrim_required=True
                )
                users.append(user_to_append)
            except:
                traceback.print_exc()
        ...
        for user in users:
            try:
                ...
                raise CommandError(
                    "Not enough permissions to apply roles to user : {}".format(
                        user.name
                    )
                )
            except:
                traceback.print_exc()
                raise CommandError(
                    "Unable to apply roles to user defined:\n{}\n".format(user.name)
                )
        response = ":thumbsup:"
        return Response(response)
    ...
    async def cmd_history(
        self, channel, message, guild, author, mentions, raw_leftover_args
    ):
        """
        ...
        if mentions:
            ...
            try:
                user = await converter.convert(
                    message, self, " ".join(raw_leftover_args)
                )
            except:
                traceback.print_exc()
        if not user:
        ...
        search_in_actions = (
            await self.do_search(
                guild_id=guild.id, channel_id=CHANS["actions"], content=user.id
            )
        )["messages"]
        for message_block in search_in_actions:
            for msg in message_block:
                if str(user.id) in msg["content"] and msg["id"] not in history_items:
                    formatted_content = (
                        cleanup_blocks(msg["content"]).strip().splitlines()
                    )

                    if (
                        len(formatted_content) == 3
                        and str(user.id) not in formatted_content[1]
                    ):
                        continue

                    history_items[msg["id"]] = {
                        "content": formatted_content,
                        "actor": discord.utils.get(
                            guild.members, id=int(msg["author"]["id"])
                        ),
                    }
        current_index = 0
        step = 10
        current_msg = None
        message_dict = collections.OrderedDict(
            sorted(history_items.items(), reverse=True)
        )
        while True:

            quick_switch_dict = {"embed": discord.Embed(), "member_obj": user}
            quick_switch_dict["embed"].set_author(
                name=f'{quick_switch_dict["member_obj"].name}({quick_switch_dict["member_obj"].id}) - {len(history_items)} {"result" if len(history_items)==1 else "results"}',
                icon_url=quick_switch_dict["member_obj"].avatar_url,
            )
            quick_switch_dict["embed"].set_footer(
                text=f"{current_index} - {(current_index+step) if (current_index+step) < len(message_dict) else len(message_dict)}"
            )
            ...
            for ts_id, msg_block in od.items():
                ...
                if not actor:
                    return CommandError(
                        "something is broke, poke rhino about it: actor not found in request via user bot"
                    )
                match = None
                if actor.bot:
                    ...
                    if match:
                        actor = discord.utils.get(
                            guild.members,
                            name=match.group(1)[:-5],
                            discriminator=match.group(1)[-4:],
                        )
                        if not actor:
                            actor = msg_block["actor"]
                acting_user_message = (
                    f"by {actor.mention}"
                    if not actor.bot
                    else f"by {actor.mention}(automated action or reason not specified)"
                )
                quick_switch_dict["embed"].add_field(
                    name=f'*{snowflake_time(ts_id).strftime("%H:%M %d.%m.%y")}*',
                    value=f'"{fmt_msg[0][7:] if not match else fmt_msg[0][7:match.start(0)]}" {acting_user_message}',
                    inline=False,
                )
    ...
    async def cmd_warn(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            if warning_str:
                ...
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000),
                    description=MSGS["warn"].format(
                        server=guild.name, reason=(": " + warning_str)
                    ),
                )
                await self.safe_send_message(user, embed=em)
                action_msg = await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        discrim=user.discriminator,
                        id=user.id,
                        action="Warned user for {}".format(warning_str),
                        reason="Action taken by {}#{}".format(
                            author.name, author.discriminator
                        ),
                    ),
                ...
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000),
                    description=MSGS["warn"].format(server=guild.name, reason=""),
                )
                await self.safe_send_message(user, embed=em)
                action_msg = await self.safe_send_message(
                    self.get_channel(CHANS["actions"]),
                    content=MSGS["action"].format(
                        ...
                        discrim=user.discriminator,
                        id=user.id,
                        action="Warned user for -REASON NOT GIVEN-",
                        reason="Action taken by {}#{}".format(
                            author.name, author.discriminator
                        ),
                    ),
    ...
    async def cmd_mute(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            if seconds_to_mute:
                ...
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000),
                    description=MUTED_MESSAGES["timed"].format(
                        time=" ".join(raw_leftover_args),
                        rules=CHANS["rules"],
                        muted=CHANS["muted"],
                    ),
                ...
                action_msg = await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        discrim=user.discriminator,
                        id=user.id,
                        action="Muted user for {}".format(" ".join(raw_leftover_args)),
                        reason="Action taken by {}#{}".format(
                            author.name, author.discriminator
                        ),
                    ),
                ...
                self.last_actions[author.id] = action_msg
                asyncio.ensure_future(
                    self.queue_timed_mute(seconds_to_mute, user, mutedrole, user.id)
                )
                ...
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000),
                    description=MUTED_MESSAGES["plain"].format(
                        rules=CHANS["rules"], muted=CHANS["muted"]
                    ),
                )
                await self.safe_send_message(user, embed=em)
                action_msg = await self.safe_send_message(
    ...
    async def cmd_slowmode(
        self, message, author, guild, channel_mentions, raw_leftover_args
    ):
        """
        ...
        if not raw_leftover_args:
            channels = [
                ...
                and channel.slowmode_delay > 0
            ]
            if channels:
                newline = "\n"
                return Response(
                    f'The following channels have slowmode enabled:{chr(10)}{chr(10)}{newline.join([f"- {channel.mention} at {channel.slowmode_delay}s" for channel in channels])}'
                )
        ...
        for channel in channels:
            if not seconds_to_slow and raw_leftover_args.startswith("0"):
                ...
                await self.safe_send_message(
                    channel, content="This channel is no longer in slow mode!"
                )
            elif seconds_to_slow and 0 < seconds_to_slow <= 21600:
                await self.safe_edit_channel(channel, slowmode_delay=seconds_to_slow)
                await self.safe_send_message(
                    channel,
                    content="The delay between allowed messages is now **%s seconds**!"
                    % seconds_to_slow,
                )
            else:
                raise CommandError(
                    "ERROR: Time provided is invalid (not between the range of 0 to 120 seconds)."
                )
            action_msg = await self.safe_send_message(
                self.get_channel(CHANS["actions"]),
                content=(MSGS["action"][:21] + MSGS["action"][57:]).format(
                    optional_content="",
                    action="Channel {} put in slow mode of {}".format(
                        channel.name, channel.slowmode_delay
                    ),
                    reason="Action taken by {}#{}".format(
                        author.name, author.discriminator
                    ),
    ...
    @mods_only
    async def cmd_botslow(
        self, message, author, guild, channel_mentions, leftover_args
    ):
        """
        ...
        try:
            if channel_mentions.id in self.slow_mode_dict.keys():
                if seconds_to_slow == 0:
                    await self.slow_mode_dict[channel_mentions.id][
                        "channel_muted_role"
                    ].delete()
                    del self.slow_mode_dict[channel_mentions.id]
                    await self.safe_send_message(
                        self.get_channel(channel_mentions.id),
                        content="This channel is no longer in slow mode!",
                    )
                else:
                    self.slow_mode_dict[channel_mentions.id][
                        "time_between"
                    ] = seconds_to_slow
                    await self.safe_send_message(
                        self.get_channel(channel_mentions.id),
                ...
                if seconds_to_slow == 0:
                    await self.safe_send_message(
                        self.get_channel(message.channel.id),
                        content="No time detected, please use a valid time!",
                    )
                    return
                slowed_channel = discord.utils.get(
                    guild.channels, id=channel_mentions.id
                )
                channel_muted_role = await guild.create_role(
                    name=slowed_channel.name + "SLOWROLE",
                    permissions=discord.Permissions(permissions=66560),
                )
                overwrite = discord.PermissionOverwrite()
                overwrite.send_messages = False
                await slowed_channel.set_permissions(
                    channel_muted_role, overwrite=overwrite
                )
                await self.safe_send_message(
                    self.get_channel(channel_mentions.id),
                    content="This channel is now in slow mode with a delay of **%s seconds**!"
                    % seconds_to_slow,
                )
                self.slow_mode_dict[channel_mentions.id] = {
                    "time_between": seconds_to_slow,
            ...
            action_msg = await self.safe_send_message(
                ...
                content=(MSGS["action"][:21] + MSGS["action"][57:]).format(
                    optional_content="",
                    action="Channel {} put in slow mode of {}".format(
                        channel_mentions.name, " ".join(leftover_args[1:])
                    ),
                    reason="Action taken by {}#{}".format(
                        author.name, author.discriminator
                    ),
    ...
    async def cmd_purge(self, message, author, guild, channel, mentions):
        ...
        parser.add_argument(
            ...
            "--u",
            nargs="+",
            help="Mentions, IDs, or Names of Target Users.^*",
        )
        parser.add_argument(
            "--contains", "--has", "--c", nargs="+", help="Contents of Msg.^*"
        )
        parser.add_argument(
            "--starts",
            "--begins",
            "--startswith",
            nargs="+",
            help="Check what Msg ends with.^*",
        )
        parser.add_argument(
            "--ends", "--endswith", nargs="+", help="Check what Msg starts with.^*"
        )
        parser.add_argument(
            "--or",
            action="store_true",
            dest="_or",
            help="Flag; Use Logical OR for all checks.^*",
        )
        parser.add_argument(
            "--not",
            action="store_true",
            dest="_not",
            help="Flag; Use Logical NOT for all checks.^*",
        )
        parser.add_argument(
            "--emoji",
            "--emotes",
            ...
            "--emote",
            action="store_true",
            help="Flag; Checks for Custom Emoji.^*",
        )
        parser.add_argument(
            "--bot",
            "--bots",
            ...
            help="Flag; Checks for Bots.^*",
        )
        parser.add_argument(
            "--embeds",
            action="store_const",
            const=lambda m: len(m.embeds),
            help="Flag; Checks for Embeds.^*",
        )
        parser.add_argument(
            "--files",
            action="store_const",
            const=lambda m: len(m.attachments),
            help="Flag; Checks for Files.^*",
        )
        parser.add_argument(
            "--reactions",
            "--reacts",
            ...
            help="Flag; Checks for Reactions.^*",
        )
        parser.add_argument(
            "--search",
            "--limit",
        ...
        try:
            deleted = await channel.purge(
                limit=args.search, before=before, after=after, check=predicate
            )
        ...
        if deleted:
            ...
            await self.log_action(
                user=author, message=deleted, action="bulk_message_delete"
            )
            deleted = len(deleted)
            messages = [
        ...
        if len(to_send) > 1990:
            return Response(
                f"Successfully removed {clean_string(deleted)} messages.",
                delete_after=10,
                delete_invoking=True,
            )
        else:
            return Response(
                clean_string(to_send), delete_after=10, delete_invoking=True
            )
    ...
    async def cmd_userinfo(
        ...
        try:
            vc_activity = await self.do_search(
                guild_id=guild.id, channel_id=CHANS["vclog"], content=user.id
            )
        except discord.HTTPException:
            raise CommandError(
                "ERROR: Bot still booting, please give me a moment to finish :)"
            )
        vc_string = ""
    ...
    async def cmd_status(self, author, message, guild, channel):
        ...
        status_colorizer = [
            value
            for value in self.server_status.values()
        ...
        if [
            ...
            if role.id in [ROLES["staff"], ROLES["tagmaster"]]
        ] or channel.id == CHANS["genbotspam"]:
            await self.safe_send_message(channel, embed=em)
    ...
    async def cmd_genembeds(self, author, message, guild, channel):
        ...
        em = discord.Embed(
            ...
            colour=discord.Colour(0x9D9BF4),
        )
        em.set_author(name="Platform Role Assignment")
        msg = await self.safe_send_message(
            self.get_channel(CHANS["roleswap"]), embed=em
        )
        ...
        em = discord.Embed(
            description="React below if you'd like to be pinged when any news regarding Rainbow 6 is posted!",
            colour=discord.Colour(0x9D9BF4),
        )
        ...
        em = discord.Embed(
            description="React below if you'd like to be pinged when any news regarding R6 Esports is posted!",
            colour=discord.Colour(0x9D9BF4),
        )
        ...
        em = discord.Embed(
            description="React below if you'd like to be pinged when any news regarding this discord server is posted!",
            colour=discord.Colour(0x9D9BF4),
        )
        ...
        await self.safe_send_message(
            self.get_channel(CHANS["roleswap"]),
    ...
    async def cmd_ban(self, message, guild, author, mentions, raw_leftover_args):
        ...
        if mentions:
            ...
            for item in raw_leftover_args:
                users.append(
                    await converter.convert(message, self, item, discrim_required=True)
                )
        ...
        for user in users:
            try:
                if ROLES["staff"] in [
                    role.id
                    for role in discord.utils.get(guild.members, id=user.id).roles
                ]:
                    return Response("Error: User is Staff!")
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000), description=MSGS["banmsg"]
                )
                await self.safe_send_message(user, embed=em)
            except:
                pass
            try:
                ...
                raise CommandError(
                    "Not enough permissions to ban user : {}".format(user.name)
                )
            except:
                traceback.print_exc()
                raise CommandError(
                    "Unable to ban user defined:\n{}\n".format(user.name)
                )
        return Response(":thumbsup:")
    ...
    async def log_action(self, user, action, *, message=None, after=None):
        ...
        if action == "server_join":
            em = discord.Embed(
                description="{0.mention} - `{0.name}#{0.discriminator} ({0.id})`".format(
                    user
                ),
                title="Account **{} old.**".format(
                    strfdelta(datetime.utcnow() - user.created_at)
                ),
                colour=discord.Colour(0x32CD32),
                timestamp=datetime.utcnow(),
            )
            em.set_author(
                name="𝅳𝅳𝅳User Joined Server", icon_url="https://i.imgur.com/cjJm2Yb.png"
            )
            em.set_thumbnail(url=user.avatar_url)
            if datetime.utcnow() - timedelta(hours=24) < user.created_at:
                em.set_footer(
                    text="WARNING: User account is < 24 hours old (FRESH)",
                    icon_url="https://i.imgur.com/RsOSopy.png",
                )
            ...
            if not user.joined_at:
                em = discord.Embed(
                    ...
                    title="Left Server",
                    colour=discord.Colour(0xCD5C5C),
                    timestamp=datetime.utcnow(),
                )
            else:
                em = discord.Embed(
                    ...
                    title="Left after **{}**.".format(
                        strfdelta(datetime.utcnow() - user.joined_at)
                    ),
                    colour=discord.Colour(0xCD5C5C),
                    timestamp=datetime.utcnow(),
                )
            ...
            em.set_author(
                name="𝅳𝅳𝅳User Left Server", icon_url="https://i.imgur.com/gturKf2.png"
            )
            em.set_thumbnail(url=user.avatar_url)
            ...
            em.set_author(
                name="𝅳𝅳𝅳User Banned from Server",
                icon_url="https://i.imgur.com/gqseDvC.png",
            )
            em.set_thumbnail(url=user.avatar_url)
            ...
            em = discord.Embed(
                ...
                colour=discord.Colour(0xFFFF99),
                timestamp=datetime.utcnow(),
            )
            em.set_author(
                name="𝅳𝅳𝅳User Unbanned from Server",
                icon_url="https://i.imgur.com/hPTzmRa.png",
            )
            em.set_thumbnail(url=user.avatar_url)
        elif action == "message_edit":
            if message.channel.id in UNLOGGED_CHANNELS:
                return
            em = discord.Embed(
                description="**𝅳𝅳𝅳User Edited Message by ID {2} in {1}**\n{0.mention} - `{0.name}#{0.discriminator} ({0.id})`".format(
                    user, message.channel.mention, message.id
                ),
            ...
            if message["channel"] in UNLOGGED_CHANNELS:
                return
            em = discord.Embed(
                description="**User's Message By ID {2} Deleted in <#{1}>**\n{0.mention} - `{0.name}#{0.discriminator} ({0.id})`".format(
                    user, message["channel"], message["message_id"]
                ),
                colour=discord.Colour(0x305EBF),
                timestamp=datetime.utcnow(),
            )
            ...
            if after:
                em.add_field(
                    name="ATTACHMENTS: ",
                    value=", ".join([attachment.url for attachment in after]),
                    inline=False,
                )

        elif action == "bulk_message_delete":
            if message[0].channel.id in UNLOGGED_CHANNELS:
                return
            actor = user
            em = []
            ...
            for user, messages in author_dict.items():
                try:
                    msgs_content = [
                        f' - `{msg.content if len(msg.content) < 1020 else msg.content[:1020]+"..."}`{"ATTACHMENTS: ```" if msg.attachments else ""}{", ".join([attachment.url for attachment in msg]) if msg.attachments else ""}{"```" if msg.attachments else ""}\n'
                        for msg in messages
                    ]
                    emb = discord.Embed(
                        description=f"**𝅳𝅳𝅳User's Messages Bulk Deleted in {message[0].channel.mention} by {actor.mention}**\n\n\n→{user.mention} - `{user.name}#{user.discriminator} ({user.id})`",
                        colour=discord.Colour(0x305EBF),
                        timestamp=datetime.utcnow(),
                    )
                    ...
                    for msg_to_send in msgs_content:
                        if count < 21:
                            emb.add_field(
                                name=f"Message {count}", value=msg_to_send, inline=False
                            )
                        count += 1
                    if count > 20:
                        emb.set_footer(
                            icon_url="https://cdn.discordapp.com/emojis/414648560110403595.gif",
                            text=f"along with {count-20} more messages.",
                        )
                    em.append(emb)
                except TypeError:
                    print(messages)
        ...
        # em.set_image(url="attachment://output.gif")
            ...
            em = discord.Embed(
                ...
                colour=discord.Colour(0xA330BF),
                timestamp=datetime.utcnow(),
            )
            em.set_author(
                name="𝅳𝅳𝅳User Changed Avatar",
                icon_url="https://i.imgur.com/C21wipj.png",
            )
            ...
            em.set_author(
                name="𝅳𝅳𝅳User Changed Name", icon_url="https://i.imgur.com/C21wipj.png"
            )
            em.set_thumbnail(url=user.avatar_url)
            ...
            em = discord.Embed(
                ...
                colour=discord.Colour(0x6130BF),
                timestamp=datetime.utcnow(),
            )
            em.set_author(
                name="𝅳𝅳𝅳User Changed Nickname",
                icon_url="https://i.imgur.com/C21wipj.png",
            )
            em.set_thumbnail(url=user.avatar_url)
            ...
            em = discord.Embed(
                ...
                colour=discord.Colour(0xC69FBA),
                timestamp=datetime.utcnow(),
            )
            em.set_author(
                name="𝅳𝅳𝅳User Changed Important Roles",
                icon_url="https://i.imgur.com/Ubp44ao.png",
            )
            em.set_thumbnail(url=user.avatar_url)
            ...
            em.add_field(
                name="BEFORE: ",
                value="`, `".join(before_values) if before_values else "None",
                inline=False,
            )
            em.add_field(
                name="\nAFTER: ",
                value="`, `".join(after_values) if after_values else "None",
                inline=False,
            )
        else:
        ...
        if isinstance(em, list):
            for emb in em:
                sent_msg = await self.safe_send_message(
                    self.get_channel(CHANS["serverlog"]), embed=emb, file=file
                )
        else:
            sent_msg = await self.safe_send_message(
                self.get_channel(CHANS["serverlog"]), embed=em, file=file
            )
        ...
        if channel_id == CHANS["roleswap"]:
            ...
            if emoji.id == REACTS["pc"]:
                await self.http.remove_role(
                    SERVERS["main"], user_id, ROLES["pc"], reason=None
                )
            if emoji.id == REACTS["xbox"]:
                await self.http.remove_role(
                    SERVERS["main"], user_id, ROLES["xbox"], reason=None
                )
            if emoji.id == REACTS["ps4"]:
                await self.http.remove_role(
                    SERVERS["main"], user_id, ROLES["ps4"], reason=None
                )
            if emoji.id == REACTS["r6ping"]:
                await self.http.remove_role(
                    SERVERS["main"], user_id, ROLES["r6news"], reason=None
                )
            if emoji.id == REACTS["esportsping"]:
                await self.http.remove_role(
                    SERVERS["main"], user_id, ROLES["esportsnews"], reason=None
                )
            if emoji.id == REACTS["invitationals"]:
                await self.http.remove_role(
                    SERVERS["main"], user_id, ROLES["invitationals"], reason=None
                )
            if emoji.id == REACTS["serverping"]:
                await self.http.remove_role(
                    SERVERS["main"], user_id, ROLES["servernews"], reason=None
                )
    ...
    async def on_guild_channel_delete(self, channel):
        if channel.id in self.cached_modmail_channels.values():
            new_cached_values = {
                key: val
                for key, val in self.cached_modmail_channels.items()
                if val != channel.id
            }
    ...
    async def on_raw_message_delete(self, payload):
        ...
        if message:
            ...
            await self.log_action(
                message=hacky_code_dict,
                after=message.attachments,
                user=message.author,
                action="message_delete",
            )
        else:
            if (
                channel_id in self.messages_log
                and message_id in self.messages_log[channel_id]
            ):
                ...
                await self.log_action(
                    message=hacky_code_dict, user=author, action="message_delete"
                )
    ...
    async def on_reaction_add(self, reaction, member):
        ...
        if reaction.message.channel.id in [CHANS["drama"]]:
            user = reaction.messagge.guild.get_member(
                self.watched_messages[reaction.message.id]["author_id"]
            )
            ...
            mutedrole = discord.utils.get(
                reaction.message.guild.roles, id=ROLES["muted"]
            )
            if reaction.emoji.id == REACTS["delete"]:
                await self.safe_delete_message(
                    await self.get_channel(
                        self.watched_messages[reaction.message.id]["channel_id"]
                    ).fetch_message(
                        self.watched_messages[reaction.message.id]["message_id"]
                    )
                )
            if reaction.emoji.id == REACTS["mute"]:
                try:
                    ...
                    await self.safe_send_message(
                        self.get_channel(CHANS["drama"]),
                        content="Cannot mute user {} ({}) for they're not on the server".format(
                ...
                if not self.use_new_modmail:
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            chan,
                            content=self.divider_content
                            + MSGS["modmailaction"].format(
                                action_log_id=CHANS["actions"],
                                username=user.mention,
                                id=user.id,
                                reason="as they were muted",
            ...
            if reaction.emoji.id == REACTS["24mute"]:
                ...
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000),
                    description=MUTED_MESSAGES["timed"].format(
                        time="24 hours", rules=CHANS["rules"], muted=CHANS["muted"]
                    ),
                )
                await self.safe_send_message(user, embed=em)
                action_msg = await self.safe_send_message(
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            ...
                            + MSGS["modmailaction"].format(
                                ...
                                username=user.mention,
                                id=user.id,
                                reason="as they were muted for 24 hrs",
                            ),
                        )
                else:
                    if target_channel:
                ...
                asyncio.ensure_future(
                    self.queue_timed_mute(86400, user, mutedrole, user.id)
                )
            if reaction.emoji.id == REACTS["48mute"]:
                ...
                em = discord.Embed(
                    colour=discord.Colour(0xFF0000),
                    description=MUTED_MESSAGES["timed"].format(
                        time="48 hours", rules=CHANS["rules"], muted=CHANS["muted"]
                    ),
                )
                await self.safe_send_message(user, embed=em)
                action_msg = await self.safe_send_message(
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            ...
                            + MSGS["modmailaction"].format(
                                ...
                                username=user.mention,
                                id=user.id,
                                reason="as they were muted for 48 hrs",
                            ),
                        )
                else:
                    if target_channel:
                ...
                asyncio.ensure_future(
                    self.queue_timed_mute(172800, user, mutedrole, user.id)
                )
            if reaction.emoji.id == REACTS["ban"]:
                try:
                    ...
                    if not user:
                        await self.safe_send_message(
                            self.get_channel(CHANS["drama"]),
                            content=MSGS["dramaerror"].format(
                                "send ban message to ",
                                self.watched_messages[reaction.message.id]["author_id"],
                                "Banning anyway...",
                            ),
    ...
    async def on_member_join(self, member):
        if member.guild.id == SERVERS["ban"]:
            return
        if (
            datetime.utcnow() - timedelta(minutes=10) < member.created_at
            and not member.avatar
        ):
            with open("kickLog.json", "r+") as f:
                data = json.load(f)
                f.seek(0)

                await member.kick(
                    reason="Account under 10 minutes old with no profile picture"
                )
        ...
        for role_id in self.channel_bans:
            if member.id in self.channel_bans[role_id]:
                if self.channel_bans[role_id][member.id]:
                    asyncio.ensure_future(
                        self.queue_timed_ban_role(
                            self.channel_bans[role_id][member.id],
                            user,
                            None,
                            role_id,
                            user.id,
                ...
                updated_member_roles = updated_member_roles + [
                    discord.utils.get(member.guild.roles, id=role_id)
                ]
        if member.id in self.muted_dict:
            updated_member_roles = updated_member_roles + [
                discord.utils.get(member.guild.roles, id=ROLES["muted"])
            ]
        else:
            ...
            if self.use_reactions:
                ...
                await self.safe_send_message(
                    self.get_channel(CHANS["registration"]), content=final_msg
                )
    ...
    async def on_member_update(self, before, after):
        if before.guild.id == SERVERS["ban"]:
            return
        ...
        if before.roles != after.roles:
            merged_roles = list((set(before.roles) - set(after.roles))) + list(
                (set(after.roles) - set(before.roles))
            )
            if [role.id for role in merged_roles if role.id not in UNPROTECTED_ROLES]:
                await self.log_action(
                    user=before, after=after, action="important_role_change"
                )
            try:
                self.serious_d_blacklist = list(set(self.serious_d_blacklist))

                if (
                    not [role for role in before.roles if role.id in [ROLES["muted"]]]
                    and [role for role in after.roles if role.id in [ROLES["muted"]]]
                    and before.id not in self.muted_dict
                ):
                    await asyncio.sleep(5)
                    ...
                    write_json("muted.json", self.muted_dict)
                if [
                    role for role in before.roles if role.id in [ROLES["muted"]]
                ] and not [role for role in after.roles if role.id in [ROLES["muted"]]]:
                    await asyncio.sleep(5)
                    ...
                    write_json("muted.json", self.muted_dict)
                if not [
                    role for role in before.roles if role.id in [ROLES["seriousd"]]
                ] and [role for role in after.roles if role.id in [ROLES["seriousd"]]]:
                ...
                if [
                    ...
                    role for role in after.roles if role.id in [ROLES["seriousd"]]
                ]:
                ...
                for role_id in self.channel_bans:
                    if (
                        not [role for role in before.roles if role.id == role_id]
                        and [role for role in after.roles if role.id == role_id]
                        and before.id not in self.channel_bans[role_id]
                    ):
                        self.channel_bans[role_id][before.id] = None
                        print("user {} now channel banned".format(before.name))
                        write_json("channel_banned.json", self.channel_bans)
                    if [role for role in before.roles if role.id == role_id] and not [
                        role for role in after.roles if role.id == role_id
                    ]:
    ...
    async def on_message_edit(self, before, after):
        if before.author == self.user:
            return
        if before.guild and before.guild.id == SERVERS["ban"]:
            return
    ...
    async def on_voice_state_update(self, member, before, after):
        if before != after:
            if (
                not (before and before.channel)
                and after
                and after.mute
                and not ROLES["staff"] in [role.id for role in member.roles]
            ):
                try:
            ...
            if member.id not in self.voice_changes:
                ...
                if (
                    ...
                    < self.voice_changes[member.id]["last_change"]
                ):
            ...
            if self.voice_changes[member.id]["changes"] < 10:
                ...
                await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        id=member.id,
                        action="Automatically Voice Muted",
                        reason="Moved voice channels 30+ times",
                    ),
                )
            ...
            if (before and before.channel) and (after and after.channel):
                await self.safe_send_message(
                    self.get_channel(CHANS["vclog"]),
                    content="{} **{}** `{}` `VCLog` `{}` **{}#{}** `{}` → `{}`".format(
                        optional_content,
                        self.voice_changes[member.id]["changes"],
                        datetime.utcnow().strftime("%y.%m.%d %H:%M:%S"),
                        member.mention,
                        clean_string(member.name),
                ...
                await self.safe_send_message(
                    self.get_channel(CHANS["vclog"]),
                    content="{} **{}** `{}` `VCLog` `{}` **{}#{}** `{}` → `<Left Voice Chat>`".format(
                        optional_content,
                        self.voice_changes[member.id]["changes"],
                        datetime.utcnow().strftime("%y.%m.%d %H:%M:%S"),
                        member.mention,
                ...
                await self.safe_send_message(
                    self.get_channel(CHANS["vclog"]),
                    content="{} **{}** `{}` `VCLog` `{}` **{}#{}** `<Joined Voice Chat>` → `{}`".format(
                        optional_content,
                        self.voice_changes[member.id]["changes"],
                        datetime.utcnow().strftime("%y.%m.%d %H:%M:%S"),
                        member.mention,
            ...
            if before and before.channel:
                ...
                if member.id in self.vc_name_locks:
                    chan_name = re.search(r"(.*)(])", before.channel.name)
                    if chan_name:
                        await before.channel.edit(name=chan_name[0])
                    self.vc_name_locks.pop(member.id, None)

                if before.channel.name.lower().startswith(("[c", "[r", "[u", "[tts")):
                    if (
                        not re.fullmatch(r"(.*)(])", before.channel.name)
    ...
    async def on_message(self, message, edit=False):
        ...
        if message.author == self.user:
            return
        elif message.webhook_id and message.channel.id == 665891802712113162:
            username = discord.utils.get(
                message.embeds[0].fields, name="What is your Discord username?"
            ).value
            try:
                usr_id = int(
                    discord.utils.get(
                        message.embeds[0].fields, name="What is your Discord user ID?"
                    ).value
                )
            except:
                await self.safe_send_message(
                    message.channel,
                    content="Provided User ID is not a number so following `userinfo` may be inaccurate...",
                )

            converter = MemberConverter()

            try:
                usr = await converter.convert(message.guild, username)
            except CommandError:
                await self.safe_send_message(
                    message.channel, content="User not in server"
                )
                return

            if usr.id != usr_id:
                await self.safe_send_message(
                    message.channel, content="Username does not match provided ID!"
                )
                return

            await self.cmd_userinfo(
                message.guild, message.channel, message, None, None, [f"{usr.id}"]
            )
            return
        ...
        if (
            self.role_ping_toggle["game"] or self.role_ping_toggle["server"]
        ) and message.role_mentions:
            if (
                self.role_ping_toggle["game"]
                and self.role_ping_toggle["game"] in message.role_mentions
            ):
                await self.role_ping_toggle["game"].edit(mentionable=False)
                self.role_ping_toggle["game"] = None
            elif (
                self.role_ping_toggle["server"]
                and self.role_ping_toggle["server"] in message.role_mentions
            ):
                await self.role_ping_toggle["server"].edit(mentionable=False)
                self.role_ping_toggle["server"] = None
            elif (
                self.role_ping_toggle["esports"]
                and self.role_ping_toggle["esports"] in message.role_mentions
            ):
        ...
        if isinstance(message.channel, discord.abc.PrivateChannel):
            member_object = discord.utils.get(
                discord.utils.get(self.guilds, id=SERVERS["main"]).members,
                id=message.author.id,
            )
            if not member_object:
                member_object = discord.utils.get(
                    discord.utils.get(self.guilds, id=SERVERS["ban"]).members,
                    id=message.author.id,
                )
            message.author = member_object
            if not self.use_new_modmail:
                if (
                    message.author.id in self.anti_spam_modmail_list
                    and self.anti_spam_modmail_list[message.author.id]["blocked"]
                ):
                    if (
                        datetime.utcnow() - timedelta(minutes=30)
                        < self.anti_spam_modmail_list[message.author.id]["last_message"]
                    ):
                        self.anti_spam_modmail_list[message.author.id][
                            "last_message"
                        ] = datetime.utcnow()
                ...
                if (
                    ...
                    < 2
                    and message.author.id not in self.anti_stupid_modmail_list
                ):
                    await self.safe_send_message(
                        message.author,
                        content=f"I noticed you're attempting to send the staff a mod mail but have no roles, if this is your issue __**PLEASE**__ make sure to review the message you recieved when you joined __along__ with reading over <#{CHANS['rules']}>! If this didn't help, please resend your original message!",
                    )
                    target_channel = await self.get_or_create_modmail_channel(
                        message.author.id, creation_state=None
                    )
                    if target_channel:
                        channel_list = [
                            self.get_channel(CHANS["modmail"]),
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            chan,
                            content=self.divider_content
                            + "→ I sent a message to {}({}) to remind them to read the welcome DM as they attempted to DM me without any roles".format(
                    ...
                    if self.anti_spam_modmail_list[message.author.id]["count"] > 4:
                        ...
                        for chan in channel_list:
                            await self.safe_send_message(
                                chan,
                                content=self.divider_content
                                + "→ I've blocked {}({}) from sending me any more mod mail as they spammed me.".format(
                                    message.author.mention, message.author.id
                                ),
                            )
                else:
                ...
                if not edit:
                    try:
                        if not (
                            message.author.id in self.recent_modmail_replies
                            and datetime.utcnow() - timedelta(minutes=30)
                            < self.recent_modmail_replies[message.author.id]
                        ):
                            await self.safe_send_message(
                                message.author,
                                content="Thank you for your message! Our mod team will reply to you as soon as possible.",
                            )
                        self.recent_modmail_replies[
                            message.author.id
                        ] = datetime.utcnow()
                        ...
                        print(
                            "ERROR: Cannot send message to user {} ({}#{})".format(
                ...
                ban_roles = [
                    role.name
                    for role in message.author.roles
                    if role.id in self.channel_bans
                ]
                msg_alert = ""
                ...
                if message.author.id in self.mod_mail_db:
                    target_channel = await self.get_or_create_modmail_channel(
                        message.author.id, creation_state="reply"
                    )
                    target_category = discord.utils.get(
                        self.get_all_channels(), id=CATS[f"mmreply"]
                    )
                    mm_symbol = "reply"
                    self.mod_mail_db[message.author.id]["messages"][
                        "{}".format(datetime_to_utc_ts(datetime.now()))
                    ] = {
                        "content": "{}{}".format(msg_content, msg_attachments),
                        "modreply": None,
                    }
                    self.mod_mail_db[message.author.id]["answered"] = False
                else:
                    target_channel = await self.get_or_create_modmail_channel(
                        message.author.id, creation_state="new"
                    )
                    target_category = discord.utils.get(
                        self.get_all_channels(), id=CATS[f"mmnew"]
                    )
                    mm_symbol = "new"
                    self.mod_mail_db[message.author.id] = {
                        "answered": False,
                        "messages": {
                            "{}".format(datetime_to_utc_ts(datetime.now())): {
                                "modreply": None,
                ...
                em = discord.Embed(
                    description=f"**{message.author.mention} - {message.author.name}#{message.author.discriminator}**\n{msg_alert}",
                ...
                if len(msg_content) > 1024:
                    ...
                    em.add_field(
                        name="─────────────",
                        value=f"{msg_edit+newline if msg_edit else ''}{msg_content}",
                    )
                if msg_attachments:
                    await self.safe_send_message(
                        target_channel,
                ...
                if target_channel and target_channel.category.id != target_category:
                    await self.safe_edit_channel(
                        target_channel,
                        category=target_category,
                        name=f"⦗{MODMAIL_SYMBOLS[mm_symbol]}{target_channel.name[2:]}",
                        sync_permissions=True,
                    )
                    asyncio.ensure_future(self.transition_modmail_channels())
                await self.safe_send_message(
                    self.get_channel(CHANS["modmail"]),
                    content=self.divider_content
                    + "**{edited_alert}From:** *{mention}*:\n```{message_content}```{attachment_info}{alert_info}\nReply ID: `{author_id}`".format(
                        edited_alert=msg_edit,
                ...
                if (
                    ...
                    em.add_field(
                        name="─────────────",
                        value=f"I noticed you're attempting to send the staff a mod mail but have no roles, if this is your issue __**PLEASE**__ make sure to review the message you recieved when you joined __along__ with reading over <#{CHANS['rules']}>! If this didn't help, please resend your original message!",
                    )
                    ...
                    if target_channel:
                        await self.safe_send_message(
                            target_channel,
                            content="→ I sent a message to {}({}) to remind them to read the welcome DM as they attempted to DM me without any roles".format(
                                message.author.mention, message.author.id
                            ),
                        )
                    self.anti_stupid_modmail_list.append(message.author.id)
                ...
                if message.author.id in self.anti_spam_modmail_list:
                    ...
                    if self.anti_spam_modmail_list[message.author.id]["count"] > 4:
                        ...
                        if target_channel:
                            await self.safe_send_message(
                                target_channel,
                                content="→ I've blocked {}({}) from sending me any more mod mail as they spammed me.".format(
                                    message.author.mention, message.author.id
                                ),
                            )
                else:
                ...
                if not edit:
                    try:
                        if not (
                            ...
                            em.add_field(
                                name="─────────────",
                                value=f"Thank you for your message! Our mod team will reply to you as soon as possible.",
                            )
                            await self.safe_send_message(message.author, embed=em)
                        self.recent_modmail_replies[
                            message.author.id
                ...
                if msg_attachments:
                    target_channel_msg = await self.safe_send_message(
                        target_channel,
                        embed=em,
                        content="\n".join(
                            [attachment.url for attachment in message.attachments]
                ...
                await self.safe_send_message(
                    self.get_channel(CHANS["modmail"]),
                    content=f"**{msg_edit}From:** *{message.author.mention}({message.author.id})*:\n```{msg_content}```{msg_attachments}",
                    embed=jump_embed,
                )
                # write_json('modmaildb.json', self.mod_mail_db)
                await self.backup_modmail_log(message.author.id)
            ...
            if message.channel.id not in self.messages_log:
                self.messages_log[message.channel.id] = {
                    message.id: {
                        "content": message.content,
                ...
                self.messages_log[message.channel.id][message.id] = {
                    "content": message.content,
                    "author": message.author.id,
                }
        ...
        try:
            this = [
                role
                for role in message.author.roles
                if role.id in [ROLES["staff"], ROLES["bots"]]
            ]
        except:
            message.author.roles = [
                role for role in message.author.roles if role is not None
            ]
        ...
        if [
            ...
            if len(message.content) < 60 or [
                mention.id
                for mention in message.mentions
                if mention.id is not message.author.id
            ]:
                await self.safe_delete_message(message)
                ...
                if message.author.id in self.mod_mail_db:
                    self.mod_mail_db[message.author.id]["messages"][
                        "{}".format(datetime_to_utc_ts(datetime.now()))
                    ] = {"content": msg_to_send, "modreply": self.user.id}
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            ...
                            content=MSGS["modmailaction"].format(
                                ...
                                id=message.author.id,
                                reason="for sending some dumb shit in teams that was shorter than 60 chars or had a mention",
                            ),
                        )
                else:
                    em = discord.Embed(
                        description=f"**{self.user.mention}**",
                        colour=message.guild.me.color,
                    )
                    em.set_thumbnail(url=self.user.avatar_url)
                    em.add_field(name="─────────────", value=msg_to_send)
                    await self.safe_send_message(message.author, embed=em)
                ...
                await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        reason="Sent some dumb shit in teams that was shorter than 60 chars or had a mention.",
                        optional_content="Message sent in {}: `{}`\n".format(
                            message.channel.mention, message.clean_content
                        ),
                    ),
                )
            ...
            if [
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            ...
                            content=MSGS["modmailaction"].format(
                                ...
                                username=message.author.mention,
                                id=message.author.id,
                                reason="for mentioning someone in lf scrims",
                            ),
                        )
                else:
                    em = discord.Embed(
                ...
                await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        id=message.author.id,
                        action="Deleted Message",
                        reason="mentioned someone in lf scrims",
                        optional_content="Message sent in {}: `{}`\n".format(
                            message.channel.mention, message.clean_content
                        ),
                    ),
            ...
            if len(message.content) > 200:
                    ...
                    for chan in channel_list:
                        await self.safe_send_message(
                            ...
                            content=MSGS["modmailaction"].format(
                                ...
                                username=message.author.mention,
                                id=message.author.id,
                                reason="for sending some dumb shit in lfg that was longer than 200 chars",
                            ),
                        )
                else:
                    em = discord.Embed(
                ...
                await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        id=message.author.id,
                        action="Deleted Message",
                        reason="Sent some dumb shit in lfg that was longer than 200 chars",
                        optional_content="Message sent in {}: `{}`\n".format(
                            message.channel.mention, message.clean_content
                        ),
                    ),
        ...
        if (
            ...
            and message.channel.id not in [CHANS["star"]]
        ):
            ...
            sent_msg = None
            if message.id in [
                self.watched_messages[msg]["message_id"]
                for msg in self.watched_messages
            ]:
                if not drama_matches and not dox_matches:
                    await self.safe_send_message(
                        self.get_channel(CHANS["drama"]),
                        content="{}#{} has edited away the past potentially drama inducing item".format(
            ...
            if drama_matches:
                em = discord.Embed(
                    description=f"**Potential Drama found in {message.channel.mention}** - [Click Here]({message.jump_url}) to jump",
                    colour=discord.Colour(0xFFFF00),
                    timestamp=datetime.utcnow(),
                )
                em.set_author(name="𝅳𝅳𝅳", icon_url="https://i.imgur.com/TVlATNp.png")
                history = reversed(
                    await message.channel.history(limit=4, before=message).flatten()
                )
                for msg in history:
                    em.add_field(
                        name="~~                    ~~",
                ...
                msg_value = (
                    message.content[: drama_matches.start()]
                    + "**__"
                    + message.content[drama_matches.start() : drama_matches.end()]
                    + "__**"
                    + message.content[drama_matches.end() :]
                )
                em.add_field(
                    name="~~                    ~~",
                    value="**%s**:\n%s" % (message.author.mention, msg_value),
                    inline=False,
                )
                sent_msg = await self.safe_send_message(
                    self.get_channel(CHANS["drama"]), embed=em
                )
            if dox_matches:
                em = discord.Embed(
                    ddescription=f"**Potential DOXXING found in {message.channel.mention}** - [Click Here]({message.jump_url}) to jump",
                    colour=discord.Colour(0xFF0000),
                    timestamp=datetime.utcnow(),
                )
                ...
                msg_value = (
                    message.content[: dox_matches.start()]
                    + "**__"
                    + message.content[dox_matches.start() : dox_matches.end()]
                    + "__**"
                    + message.content[dox_matches.end() :]
                )
                em.add_field(
                    name="~~                    ~~",
                    value="**%s**:\n%s" % (message.author.mention, msg_value),
            ...
            if sent_msg:
                self.watched_messages[sent_msg.id] = {
                    "author_id": message.author.id,
                    "message_id": message.id,
        ...
        if not message.author.bot and not [
            ...
            if len(message.mentions) > 9 and not [
                item
                for item in message.content.strip().split()
                if item not in [msg_author.mention for msg_author in message.mentions]
            ]:
                ...
                await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        id=message.author.id,
                        action="Banned user",
                        reason="Mention Spam in excess of 10 mentions per message",
                    ),
                )
                ...
                await self.safe_send_message(
                    message.author,
                    content=MUTED_MESSAGES["plain"].format(
                        rules=CHANS["rules"], muted=CHANS["muted"]
                    ),
                )
                await self.safe_send_message(
                    ...
                    content=MSGS["action"].format(
                        ...
                        id=message.author.id,
                        action="Muted user",
                        reason="Mention Spam in excess of 5 mentions per message",
                    ),
                )
                target_channel = await self.get_or_create_modmail_channel(
                    message.author.id, creation_state=None
                ...
                len(message.mentions) > 2
                and message.author.id in self.mention_spam_watch
                and not edit
            ):
                if (
                    datetime.utcnow() - timedelta(minutes=30)
                    < self.mention_spam_watch[message.author.id]
                ):
                    ...
                    await self.safe_send_message(
                        ...
                        content=MSGS["action"].format(
                            ...
                            id=message.author.id,
                            action="Muted user",
                            reason="Mention Spam in excess of 3 mentions per message more than once",
                        ),
                    )
                    target_channel = await self.get_or_create_modmail_channel(
                        message.author.id, creation_state=None
                ...
                if "discord.gg" in item or "discordapp.com/invite" in item:
                    ...
                    if invite.guild.id not in self.guild_whitelist:
                        if not [
                            ...
                            await self.safe_delete_message(message)
                            print(
                                "detected illegal invite from {}:{}\t{}".format(
                                    message.author.name, message.author.id, item
                                )
                            )
                            msg_to_send = "I've deleted your message in {} since I detected an invite url in your message! Please remember not to advertise servers not approved by staff!".format(
                            ...
                            if message.author.id in self.mod_mail_db:
                                ...
                                self.mod_mail_db[message.author.id] = {
                                    ...
                                    "messages": {
                                        "{}".format(
                                            datetime_to_utc_ts(datetime.now())
                                        ): {
                                            "modreply": self.user.id,
                                            "content": msg_to_send,
                                        }
                            ...
                            await self.safe_send_message(
                                message.author, content=msg_to_send
                            )
                                ...
                                for chan in channel_list:
                                    await self.safe_send_message(
                                        ...
                                        content=MSGS["modmailaction"].format(
                                            ...
                                            username=message.author.mention,
                                            id=message.author.id,
                                            reason="for sending discord invites",
                                        ),
                                    )
                            else:
                                em = discord.Embed(
                            ...
                            await self.safe_send_message(
                                ...
                                content=MSGS["action"].format(
                                    username=message.author.name,
                                    action="Deleted discord invite",
                                    discrim=message.author.discriminator,
                                    id=message.author.id,
                                    reason="Sent a nonwhitelisted invite url ({} : {})".format(
                                        item, invite.guild.id
                                    ),
                                    optional_content="Message sent in {}: `{}`\n".format(
                                        message.channel.mention, message.clean_content
                                    ),
        ...
        if not message_content.startswith(self.prefix) or edit:
            if message.channel.id in list(self.slow_mode_dict.keys()):
                ...
                if self.slow_mode_debug:
                    print(
                        f"putting {message.author.name} in slow mode for {time_out} seconds in no command method"
                    )
                channel_muted_role = self.slow_mode_dict[message.channel.id][
                    "channel_muted_role"
                ]
                if self.slow_mode_debug:
                    print(f"adding role...")
        ...
        if not handler:
            ...
            if class_name.lower() in ROLE_ALIASES:
                role = discord.utils.get(
                    message.guild.roles, id=ROLE_ALIASES[class_name.lower()]
                )
            else:
                role = discord.utils.get(message.guild.roles, name=class_name.lower())
            if role and message.channel.id in [
                CHANS["genbotspam"],
                CHANS["registration"],
            ]:
                author_roles = message.author.roles
                mod_check = [
                    role for role in author_roles if role.id not in UNPROTECTED_ROLES
                ]
                author_roles.append(role)
                if not ROLES["staff"] in [role.id for role in message.author.roles]:
                    await message.author.edit(roles=author_roles)
                await self.safe_send_message(
                    message.channel,
                    content="%s, you now are marked with the role `%s`!"
                    % (message.author.mention, role.name),
                    expire_in=15,
                )
                print(
            ...
            if tag_name in self.tags:
                if self.tags[tag_name][0]:
                    ...
                    for item in tag_flags:
                        if item.isdigit() and message.guild.get_channel(int(item)):
                            ...
                            if isinstance(chan, discord.CategoryChannel):
                                ...
                                acceptable_chans.append(
                                    (message.guild.get_channel(int(item))).id
                                )
                    if (
                        message.channel.id not in acceptable_chans
                        and len(acceptable_chans) > 0
                    ):
                        await self.safe_send_message(
                            message.channel,
                            content=f'Tag cannot be used outside of {", ".join([f"<#{chan}>" for chan in acceptable_chans])}',
                            expire_in=20,
                        )
                        return
                    ...
                    if "unrestricted_eval" in tag_flags:
                        ...
                        if resp:
                            await self.safe_send_message(
                                message.channel, content=clean_bad_pings(resp)
                            )
                        return
                    elif "restrict" in tag_flags and not [
                        role
                ...
                await self.safe_send_message(
                    message.channel, content=clean_bad_pings(self.tags[tag_name][1])
                )
        ...
        try:
            ...
            if response and isinstance(response, Response):
                ...
                if response.delete_invoking:
                    await self.safe_delete_message(message)
                elif response.delete_with_expire:
                ...
                if response.delete_after > 0:
                    sentmsg = await self.safe_send_message(
                        ...
                        delete_with_expire=msg_to_delete,
                    )
                else:
                    sentmsg = await self.safe_send_message(
                        message.channel, content=content, embed=embed
                    )
