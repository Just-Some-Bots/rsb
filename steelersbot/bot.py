...
from .creds import BOT_TOKEN, SLEEPER_TOKEN, USER_TOKEN, TWITTER_CON_KEY, TWITTER_CON_SECRET, TWITTER_TOKEN_KEY, TWITTER_TOKEN_SECRET
...
class SteelersBot(discord.Client):
    def __init__(self):
        ...
        self.scheduler.add_job(self.refresh_player_data, 'cron', id='refresh_player_data',  minute='*/10')
        self.scheduler.add_job(self.refresh_team_data, 'cron', id='refresh_team_data',  day='*/1')
    ...
    def run(self):
        ...
        try:
            ...
            loop.create_task(self.check_sleeper_feeds())
    ...
    async def queue_timed_ban_role(self, sec_time, user, timed_role, role_id, user_id):
        ...
        try:
            ...
            if timed_role in user.roles:
                if not ROLES['moderators'] in [role.id for role in user.roles] and not user.bot: await user.remove_roles(timed_role, atomic = True)
        except:
            traceback.print_exc()
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.get(str(resp["stream"]["preview"]["large"])) as r:
                ...
                with open("img.png", "wb") as f:
                    f.write(data)
        file = discord.File("/home/bots/steelersbot/img.png", filename="img.png")
    ...
    async def refresh_player_data(self):
        ...
        async with aiohttp.ClientSession() as session:
            resp = None
            async with session.get('https://api.sleeper.app/v1/players/nfl') as r:
                resp = await r.json()
                if resp:
                    print("successfully got player data")
                    self.player_data = resp
    ...
    async def refresh_team_data(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            resp = None
            headers = {"authorization": SLEEPER_TOKEN, "content-type": "application/json"}
            async with session.post('https://sleeper.app/graphql', json=TEAMS_SLEEPER_QUERY, headers=headers) as r:
                resp = await r.json()
                if resp:
                    self.team_data = {team["team"]: team for team in resp["data"]["teams"]}
    
    async def fetch_tweet_by_id(self, tweet_id):
        ...
        future = loop.run_in_executor(self.executor, get_twitter_data, tweet_id)
    ...
    async def check_sleeper_feeds(self):
        await self.wait_until_really_ready()
        target_channel = discord.utils.get(self.get_all_channels(), id=CHANS["feed"])
        if self.sleeper_feed_debug: print("init sleeper feed")
        most_recent_timestamp = (await target_channel.history(limit=1).flatten())[0].created_at.timestamp()
        most_recent_timestamp = most_recent_timestamp * 1000 # because sleeper represents their timestamps in milliseconds
        if self.sleeper_feed_debug: print(f"most recent timestamp {most_recent_timestamp}")
        while True:
            try:
                async with aiohttp.ClientSession() as session:
                    await asyncio.sleep(15)
                    if self.sleeper_feed_debug: print("starting this periods check...")
                    resp = None
                    if not self.player_data or not self.team_data:
                        if self.sleeper_feed_debug: print("no player or team data")
                        continue
                    headers = {"authorization": SLEEPER_TOKEN, "content-type": "application/json"}
                    async with session.post('https://sleeper.app/graphql', json=GEN_SLEEPER_QUERY, headers=headers) as r:
                        try:
                            resp = await r.json()
                        except json.decoder.JSONDecodeError:
                            pass
                    if resp:
                        if self.sleeper_feed_debug: print("got resp")
                        if "errors" in resp or "data" not in resp:
                            print(f"error within sleeper feed {resp}")
                            await asyncio.sleep(600)
                            continue
                        topic_urls = [(topic["created"], topic) for topic in resp["data"]["topics"] if topic["created"] > most_recent_timestamp]
                        topic_urls = sorted(topic_urls, key=lambda x: x[0])
                        if self.sleeper_feed_debug: print(f"{len(topic_urls)} detected topics need to be posted")
                        for topic_tuple in topic_urls:
                            if self.sleeper_feed_debug: print(topic_tuple)
                            em = await self.generate_sleeper_embed(topic_tuple[1])
                            await self.safe_send_message(target_channel, embed=em)
                            most_recent_timestamp = topic_tuple[0]
            except RuntimeError:
                traceback.print_exc()
                return
            except:
                traceback.print_exc()
                # print(r)
                print('error within sleeper feed, handled to prevent breaking')
                
    async def generate_sleeper_embed(self, sleeper_resp):
        resp_title_map = list(sleeper_resp["title_map"].values())[0]
        final_text = clean_string(resp_title_map["data"]["info"]["description"])
        
        players = [self.player_data[player_id.split("-").pop()] for player_id in sleeper_resp["player_tags"]]
        target_player = players[0]
        
        date_time = datetime.fromtimestamp(sleeper_resp["created"] / 1000)
        em = discord.Embed(colour=discord.Colour(int(self.team_data[target_player["team"]]["metadata"]["color1"], 16) if target_player["team"] else 0x01081f), description=final_text, timestamp=date_time)
        thumb_url = "https://sleepercdn.com/content/nfl/players/{}.jpg".format(target_player['player_id']) if target_player['player_id'] else print("no playerid")
        ...
        while True:
            ...
            while ticker != 1800:
                ...
                if not [member_id for member_id in self.mod_mail_db if not self.mod_mail_db[member_id]['answered']]:
                    ...
                    ticker+=1
            try:
                async for lmsg in self.get_channel(CHANS['modchat']).history(limit=1):
    ...
    async def get_tweets(self):
        ...
        while True:
            try:
                if not self.twitter_watch_list:
                    await asyncio.sleep(600)
                    continue
                for handle in self.twitter_watch_list:
    ...
    async def generate_new_tweet_embed(self, resp):
        ...
        if "is_quote_status" in resp and resp["is_quote_status"] and "quoted_status" in resp and resp["quoted_status"]:
                        ...
                        if resp and "stream" in resp and resp["stream"]:
                            ...
                            if streamer not in self.twitch_is_live:
                                ...
                                self.twitch_is_live[streamer] = {'detected_start': datetime.utcnow(),
                                                                 ...
                                                                 'message': None}
                                self.twitch_is_live[streamer]['message'] = await self.safe_send_message(self.get_channel(target_channel), embed=self.twitch_is_live[streamer]['embedded_object'], file=discord.File("/home/bots/steelersbot/img.png", filename="img.png"))
                            else:
    ...
    async def on_ready(self):
        ...
        print('Done!\n\nFetching NFL Team Data...')
        await self.refresh_team_data()
        ...
        for user_id, timestamp in temp_dict.items():
            ...
            if user:
                try:
                    if not ROLES['moderators'] in [role.id for role in user.roles] and not user.bot: 
        ...
        await self.safe_send_message(self.get_channel(CHANS['modchat']), content='I have just been rebooted!')
    ...
    def mods_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if [role for role in orig_msg.author.roles if role.id  in [ROLES['moderators']]] or (is_tag and (self.tags[is_tag][0] == 'unrestricted_eval' or [role for role in orig_msg.author.roles if role.id  in [ROLES['moderators']]])):
    ...
    async def cmd_clear(self, message, author, channel):
        ...
        if mod_check:
            ...
            author_roles = []
        if not ROLES['moderators'] in [role.id for role in author.roles]: await author.edit(roles = author_roles)
    ...
    async def cmd_tag(self, message, guild, author, channel, mentions, leftover_args, eval_content):
        ...
        if switch in ['+', 'add', '-', 'remove', 'list', 'blacklist']:
            if switch in ['+', 'add']:
                if [role for role in author.roles if role.id  in [ROLES['moderators']]]:
                ...
                if tag_name.lower() == tag.lower():
                    if self.tags[tag][0]:
                        ...
                        if "unrestricted_eval" in tag_flags:
                            ...
                            return
                        elif "restrict" in tag_flags and not [role for role in author.roles if role.id  in [ROLES['moderators']]]:
                            return
    ...
    async def cmd_cmdinfo(self, author, command=None):
        ...
        if command:
            ...
            helpmsg += "```\n"
            helpmsg += "You can also use `{}cmdinfo x` for more info about each command.".format(self.prefix)
    ...
    async def cmd_unmute(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                ...
                if not ROLES['moderators'] in [role.id for role in user.roles] and not user.bot: await user.edit(roles = [role for role in user.roles if role.id not in UNPROTECTED_ROLES])
                try:
                    await user.edit(mute=False)
                except:
                    pass
    ...
    async def cmd_cban(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                user = guild.get_member(user.id)
                if not ROLES['moderators'] in [role.id for role in user.roles] and not user.bot: await user.edit(roles = list(set(([role for role in user.roles] + [role]))))
            except discord.Forbidden:
    ...
    async def cmd_mute(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                user = guild.get_member(user.id)
                if not ROLES['moderators'] in [role.id for role in user.roles] and not user.bot: await user.edit(roles = list(set(([role for role in user.roles if role.id not in UNPROTECTED_ROLES] + [mutedrole]))))
    ...
    async def cmd_userinfo(self, guild, channel, message, author, mentions, raw_leftover_args):
        ...
        if all([True if role.id not in [ROLES['moderators']] else False for role in author.roles ]) and member.id != author.id:
    ...
    async def cmd_genembeds(self, author, message, guild, channel):
        ...
        em = discord.Embed(description="React below if you're part of Steelers Nation", colour=discord.Colour(0xFFB612))
        em.set_author(name="Steelers Nation Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['steelers']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the AFC East", colour=discord.Colour(0xCE1126))
        em.set_author(name="AFC East Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['bills']))
        await msg.add_reaction(self.get_emoji(REACTS['dolphins']))
        await msg.add_reaction(self.get_emoji(REACTS['patriots']))
        await msg.add_reaction(self.get_emoji(REACTS['jets']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the AFC North", colour=discord.Colour(0xCE1126))
        em.set_author(name="AFC North Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['ravens']))
        await msg.add_reaction(self.get_emoji(REACTS['browns']))
        await msg.add_reaction(self.get_emoji(REACTS['bengals']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the AFC South", colour=discord.Colour(0xCE1126))
        em.set_author(name="AFC South Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['texans']))
        await msg.add_reaction(self.get_emoji(REACTS['colts']))
        await msg.add_reaction(self.get_emoji(REACTS['jaguars']))
        await msg.add_reaction(self.get_emoji(REACTS['titans']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the AFC West", colour=discord.Colour(0xCE1126))
        em.set_author(name="AFC West Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['broncos']))
        await msg.add_reaction(self.get_emoji(REACTS['chiefs']))
        await msg.add_reaction(self.get_emoji(REACTS['chargers']))
        await msg.add_reaction(self.get_emoji(REACTS['raiders']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the NFC East", colour=discord.Colour(0x003B66))
        em.set_author(name="NFC East Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['cowboys']))
        await msg.add_reaction(self.get_emoji(REACTS['giants']))
        await msg.add_reaction(self.get_emoji(REACTS['eagles']))
        await msg.add_reaction(self.get_emoji(REACTS['redskins']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the NFC North", colour=discord.Colour(0x003B66))
        em.set_author(name="NFC North Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['bears']))
        await msg.add_reaction(self.get_emoji(REACTS['lions']))
        await msg.add_reaction(self.get_emoji(REACTS['packers']))
        await msg.add_reaction(self.get_emoji(REACTS['vikings']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the NFC South", colour=discord.Colour(0x003B66))
        em.set_author(name="NFC South Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['falcons']))
        await msg.add_reaction(self.get_emoji(REACTS['panthers']))
        await msg.add_reaction(self.get_emoji(REACTS['saints']))
        await msg.add_reaction(self.get_emoji(REACTS['buccaneers']))
        
        await asyncio.sleep(0.1)
        
        em = discord.Embed(description="React below to your teams logo if you're representing a team from the NFC West", colour=discord.Colour(0x003B66))
        em.set_author(name="NFC West Team Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['cardinals']))
        await msg.add_reaction(self.get_emoji(REACTS['rams']))
        await msg.add_reaction(self.get_emoji(REACTS['49ers']))
        await msg.add_reaction(self.get_emoji(REACTS['seahawks']))
    ...
    async def cmd_ban(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                if  ROLES['moderators'] in [role.id for role in discord.utils.get(guild.members, id=user.id).roles]: return Response('Error: User is Staff!')
    ...
    async def cmd_softban(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            ...
            # except:
                # pass
            try:
                if  ROLES['moderators'] in [role.id for role in user.roles]: return Response('Error: User is Staff!')
    ...
    async def log_action(self, user, action, *, message=None, after = None):
        ...
        # elif action == 'avatar_change':
            ...
            # if ffmpeg_success:
                ...
                # file = discord.File("/home/bots/steelersbot/output.gif", filename="output.gif")
    ...
    async def on_raw_reaction_remove(self, payload):
        ...
        if channel_id == CHANS['roleswap']:
            ...
            if member and [role for role in member.roles if role.id == ROLES['muted']]:
                return
            if emoji.id in ROLE_REACT_MAP:
                await self.http.remove_role(SERVERS['main'], user_id, ROLE_REACT_MAP[emoji.id], reason=None)
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if channel_id == CHANS['roleswap']:
            ...
            if member and [role for role in member.roles if role.id == ROLES['muted']]:
                return
            if emoji.id in ROLE_REACT_MAP:
                await self.http.add_role(SERVERS['main'], user_id, ROLE_REACT_MAP[emoji.id], reason=None)
    ...
    async def on_reaction_add(self, reaction, member):
        ...
        if reaction.message.channel.id in [CHANS['drama']]:
            ...
            if ROLES['moderators'] in [role.id for role in user.roles]: 
            ...
            if reaction.emoji.id == REACTS['ban']:
                try:
                    if  ROLES['moderators'] in [role.id for role in user.roles]: return
                    em = discord.Embed(colour=discord.Colour(0xFF0000), description=MSGS['banmsg'])
                    await self.safe_send_message(user, embed=em)
                except:
                    if not user:
    ...
    async def on_member_join(self, member):
        ...
        if updated_member_roles:
            if not ROLES['moderators'] in [role.id for role in member.roles]:
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before != after:
            if not (before and before.channel) and after and after.mute and not ROLES['moderators'] in [role.id for role in member.roles]:
                try:
            ...
            optional_content = ''
            staff_role = False
            role = discord.utils.get(member.guild.roles, id=ROLES['moderators'])
            ...
            if self.voice_changes[member.id]['changes'] < 10:
                ...
                try:
                    ...
                    pass
                if not ROLES['moderators'] in [role.id for role in member.roles]: await member.edit(roles = member_roles) 
            else:
    ...
    async def on_message(self, message, edit=False):
        ...
        try:
            this = [role for role in message.author.roles if role.id  in [ROLES['moderators'], ROLES['bots']]]
        except:
            message.author.roles = [role for role in message.author.roles if role is not None]
            
        if not message.author.bot and not [role for role in message.author.roles if role.id  in [ROLES['moderators'], ROLES['bots']]]:
            ...
            try:
                if 'discord.gg' in item or 'discordapp.com/invite' in item:
                    ...
                    if invite.guild.id not in self.guild_whitelist:
                        if not [role for role in message.author.roles if role.id  in [ROLES['moderators'], ROLES['bots']]]:
                            await self.safe_delete_message(message)
            ...
            if tag_name in self.tags:
                if self.tags[tag_name][0]:
                    ...
                    if "unrestricted_eval" in tag_flags:
                        ...
                        return
                    elif "restrict" in tag_flags and not [role for role in message.author.roles if role.id  in [ROLES['moderators']]]:
                        return
                    elif "eval" in tag_flags:
                        resp = await self.cmd_eval(message.author, message.guild, message, message.channel, message.mentions, self.tags[tag_name][1], is_origin_tag=True)
                        if resp:
