...
class Jeeves(discord.Client):
    def __init__(self):
        ...
        self.scheduler.add_job(self.reorder_lfg_channels, 'cron', id='reorder_lfg',  second='*/15')
    ...
    async def voice_channel_resizing(self):
        while True:
            ...
            for category_id in self.vc_tracker:
                ...
                if baseline_channel_bucket < 4:
                    for _ in range(4-baseline_channel_bucket):
                ...
                if empty_channel_bucket < 2:
                    for _ in range(2-empty_channel_bucket): 
                        await self.queue_channel_for_creation(category_id)
    ...
    async def queue_channel_for_creation(self, category_id):
        ...
        await chan.edit(user_limit=(8 if cat.id == CATS['tf'] else 3), bitrate=128000)
    ...
    async def on_ready(self):
        ...
        self.vc_categories = {'apex': self.get_channel(CATS['apex']),
                              'tf': self.get_channel(CATS['tf'])}
    ...
    def double_prefix(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if f"{self.prefix}{self.prefix}" in prefix:
    ...
    async def cmd_shrink(self, message, channel, author):
        ...
        if author.voice and author.voice.channel and author.voice.channel.category.id in self.vc_tracker:
            if author.id not in self.vc_num_locks:
                ...
                await author.voice.channel.edit(user_limit=(8 if author.voice.channel.category_id == CATS['tf'] else 3))
    ...
    # async def cmd_reason(self, message, guild, author, mentions, message_id, leftover_args):
        ...
        # for msg_id in msg_ids:
            ...
            # await self.safe_delete_message(message)
        # return Response(':thumbsup:', delete_after=10)
    ...
    # async def on_typing(self, channel, user, when):
        # if channel.id in [CHANS['gamenews'], CHANS['servernews']]:
            # self.role_ping_toggle['game'] = discord.utils.get(channel.guild.roles, id=ROLES['gamenews'])
            # self.role_ping_toggle['server'] = discord.utils.get(channel.guild.roles, id=ROLES['servernews'])
            # await self.role_ping_toggle['server'].edit(mentionable=True)
            # await self.role_ping_toggle['game'].edit(mentionable=True)
    ...
    async def on_voice_state_update(self, member, before, after):
            if (before and before.channel):
                ...
                if member.id in self.vc_num_locks:
                    await before.channel.edit(user_limit=(8 if author.voice.channel.category_id == CATS['tf'] else 3))
